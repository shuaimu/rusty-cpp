...
pub fn check_mutable_fields(
    ...
    debug_println!(
        "MUTABLE: Checking {} classes for mutable fields",
        ast.classes.len()
    );
    ...
    for class in &ast.classes {
        debug_println!(
            "MUTABLE: Checking class '{}' with {} members",
            class.name,
            class.members.len()
        );
        ...
        if let Some(ext_annot) = external_annotations {
            if ext_annot.is_type_unsafe(&class.name) {
                debug_println!(
                    "MUTABLE: Class '{}' is an unsafe_type - skipping internal analysis",
                    class.name
                );
                continue;
            }
        ...
        for member in &class.members {
            debug_println!(
                "MUTABLE: Checking member '{}' is_mutable = {}",
                member.name,
                member.is_mutable
            );
            if member.is_mutable {
                let error = format!(
                    ...
                    member.location.file, member.location.line, member.name, class.name
                );
                errors.push(error);
            }
        }
...
fn is_class_safe(class: &Class, safety_context: &SafetyContext) -> bool {
    ...
    debug_println!(
        "MUTABLE: Class '{}' from '{}' has safety mode: {:?}",
        class.name,
        class_file,
        class_safety
    );
    ...
    if class_safety == SafetyMode::Safe {
        debug_println!(
            "MUTABLE: Class '{}' is marked @safe - checking for mutable fields",
            class.name
        );
        return true;
    }
    ...
    debug_println!(
        "MUTABLE: Class '{}' is @unsafe - mutable fields allowed",
        class.name
    );
    false
}
