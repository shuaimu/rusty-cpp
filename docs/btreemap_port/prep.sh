...
// Constants previously in this file; preserved verbatim.
...
pub(super) const MIN_LEN: usize = B - 1;
MOD
  ...
  # And rewrite the same references inside the merged file itself —
  # any `super::map::FOO` reference in the original orphan-impl
  # files needs to find `FOO` declared in mod.rs (one level up).
  # `MIN_LEN` is the only case we know about; check for others by
  # running the build.
  sed -i 's|use super::map::MIN_LEN;|use super::MIN_LEN;|g' "$INTERNAL"
