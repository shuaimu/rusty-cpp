#!/usr/bin/env python3
"""Post-transpile patches for vec_deque_tests_port.

Currently the transpiled output of `library/alloctests/tests/vec_deque.rs`
...
`transpiled/vec_deque_tests_port/vec_deque_tests_port.cppm` is a hand-stub
...
To regenerate the stub:
    python3 docs/_gen_test_stub.py \
        ~/.rustup/.../library/alloctests/tests/vec_deque.rs \
        vec_deque_tests_port \
        transpiled/vec_deque_tests_port/vec_deque_tests_port.cppm

To switch to a fully-transpiled cppm, transpile + run this script:
    bash docs/vec_deque_tests_port/prep.sh <tgt>/src/lib.rs
