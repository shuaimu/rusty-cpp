...
- [Chapter 6 — Tier 2 collection ports (BinaryHeap / VecDeque / LinkedList)](#chapter-6--tier-2-collection-ports-binaryheap--vecdeque--linkedlist)
  ...
  - [6.11 Universal auto-deref dispatcher (`rusty::deref_call`)](#611-universal-auto-deref-dispatcher-rustyderef_call)
  - [6.12 Guard flow — deciding when the invisible deref exists](#612-guard-flow--deciding-when-the-invisible-deref-exists)
- [Chapter 7 — Codegen correctness: recurring translation-semantics bug classes](#chapter-7--codegen-correctness-recurring-translation-semantics-bug-classes)
  ...
  - [7.14 Class G1 — invisible autoderef through guards](#714-class-g1--invisible-autoderef-through-guards)
...
`include/rusty/dispatch.hpp` provides the universal form (current
signature — the call's arguments are forwarded through the dispatcher):
...
constexpr decltype(auto) deref_call(R&& r, F&& f, A&&... a) {
    if constexpr (requires { f(r, a...); }) {
        ...
        // constraint guarantees *r is well-formed and reachable
...
**Caller shape (current).** The transpiler emits one *namespace-scope
functor* per method name, generated by `RUSTY_METHOD_DISPATCH(name)`:

```cpp
namespace rusty { namespace detail {
RUSTY_METHOD_DISPATCH(push_back)   // defines struct __mdisp_push_back
} }

rusty::deref_call(h.q.borrow_mut(), rusty::detail::__mdisp_push_back{}, 1);
```

The functor's `operator()` carries the SFINAE-friendly
`-> decltype(recv.name(args...))` trailing return, so a step of the
chain that lacks the method makes `requires { f(r, a...); }` cleanly
fail and the dispatcher derefs one level and retries.

> **Why a functor and not a lambda.** The original design passed a
> generic lambda local to the enclosing function template, capturing
> the call's arguments. That exact shape — a local generic lambda whose
> closure type crosses a C++20 module (BMI) boundary — crashes clang's
> Itanium mangler (issue #31, reproduced on clang 21/22). Namespace-
> scope functors plus argument forwarding through `deref_call`'s
> trailing pack avoid the shape entirely. If you hand-write a
> dispatcher call, use the functor form.

`decltype(auto)` return on `deref_call` preserves value category —
methods that return `T&` stay `T&`, methods that return `T` stay `T`.

Chained calls (`a.b().c()`) compose by nesting `deref_call`s, each with
its own functor.
...
3. **Uncertain receiver** (fallback): when the receiver type is a
   template parameter, an associated-type projection, or a value the
   inference cannot see through — emit
   `rusty::deref_call(r, rusty::detail::__mdisp_method{}, args...)`.
   Pays template-instantiation cost but is *correct* regardless of
   what `r`'s actual type and deref chain turn out to be after
   monomorphization.

Which tier a given expression lands in is no longer decided ad hoc at
each emission site: §6.12 describes the classifier that owns the
decision. The per-method free helpers (`rusty::iter`, `rusty::len`,
...) remain as the routing targets for method-specific *semantics*
(Rust's `get` returns `Option<&T>`, not `T&`); each ends with a
deref-peel arm so they stay wrapper-tolerant — also §6.12.
...
`std::invoke`. Its own safety should propagate from the callee's body,
not be asserted on the dispatcher template.
...
real safety violations in the caller's body walk — the dispatch
functor's body forwards to a method call that is part of the caller's
own translation unit and is analyzed in the caller's `@safe` context.
...
```cpp
namespace rusty { namespace detail { RUSTY_METHOD_DISPATCH(sum) } }

// @safe
int sum(rusty::Vec<int> const& v) {
    return rusty::deref_call(v, rusty::detail::__mdisp_sum{});
}
```

(Other natural bridge candidates: smart-pointer `operator->`
overloads, `std::invoke`-shaped helpers, and the free helpers
`rusty::iter` / `rusty::len` / `rusty::detail::deref_if_pointer_like`.)

### 6.12 Guard flow — deciding when the invisible deref exists

§6.11 gives the *mechanism* (a dispatcher that walks `operator*` at
C++ compile time). This section is the *policy*: how the transpiler
decides, per expression, that an invisible Rust deref exists at all.
It exists because issues #32, #33, #34 and #35 were all one defect —
`borrow_mut()` yields a **guard object** in the C++ runtime while the
type model said `&mut T`, and Rust's autoderef means the source
contains no `*` to translate — surfacing one syntactic context at a
time. Per-report fixing left the next context as the next issue.

The normative reference is `docs/guard_autoderef.md`; the code is one
module, `transpiler/src/codegen/guard_flow.rs`. The shape:

**Two tiers by knowledge.**

- *Typed tier* — inference sees the type. Crate types are handled
  structurally: any type with an `impl Deref`/`DerefMut` (collected
  into `user_deref_targets`, replayed across sibling files so the
  answer never depends on file emission order) gets rustc-style
  resolution — a method on the Target emits `(*g).method()`, an
  inherent method stays `g.method()`. For the hand-written runtime
  cell/lock types, `known_guard_producing_call()` is the one predicate
  saying "this yields a guard BY VALUE" (binding logic must not emit
  `auto&` against the temporary; concrete lowerings emit `->`).
- *Untyped tier* — inference cannot see the type. Classification is by
  **value flow, not syntax**: the guard call itself; `unwrap()`/
  `expect()` peeling a `Result<Guard, _>`; a call whose *declared
  return type* is a deref wrapper (the signature is the flow edge); a
  local bound from any of these (bound `auto&&`, remembered, including
  behind `if`/`match`/block-tail initializers and element-wise through
  tuple destructures). Consumption then goes through the tolerant
  forms, correct whichever the value turns out to be: method calls via
  the §6.11 dispatcher, place/value uses (`*g`, `*g = v`, `*g += v`)
  via `rusty::detail::deref_if_pointer_like`, free-helper arguments
  via the helper's own final deref-peel arm
  (`else if constexpr (requires { *x; }) { return helper(*x); }`).

**The trait impl is the signal, not a name.** The classifier was
originally name-driven (`borrow`/`lock` producer names, guard type
tails) — the wrong endpoint, since Rust's actual signal is `impl
Deref` and a custom guard appears on no list. It now consults the
collected impls; a name seed survives only for the runtime guards
(`Ref`, `RefMut`, `MutexGuard`, …) whose Rust-side impl the transpiler
never sees — the role rustc's std metadata plays — and the producer
names remain only as the last-resort heuristic for receivers so opaque
that no signature exists at all.

**The scoping lesson that cost a red gate:** in Rust *everything*
implements `Deref` — `Vec`, `String`, `SmallVec`. Classifying every
`impl Deref` type unconditionally re-routed perfectly good typed
emission through the tolerant machinery (one golden test plus the
alloc and smallvec matrix crates went red). The rule: a *seed* guard
classifies unconditionally (the typed tier has no dispatch for the
hand-written guards), a *crate* Deref type only when the expression is
untypable — when the type is visible, the typed tier wins.

**Rules that keep it sound** (numbered doctrine in `guard_flow.rs`):

1. Classify flow, not shape — every path a guard can travel must give
   the same answer.
2. Every shape-laddered runtime helper ends with a deref-peel arm; a
   permissive `return false`-style tail *hides silent-wrongs* (a guard
   falls through every probe and the tail invents an answer —
   `rusty::contains` and the emitted `is_empty` both did this).
3. Fire only on untypable receivers; the typed tier emits better code.
4. The dispatcher is only sound if runtime members are Rust-faithful:
   `deref_call` calls whatever member it finds, with that member's
   semantics. (`VecDeque::front()` returning `T&` instead of
   `Option<&T>` broke both tiers; the fix is the member, never emitter
   compensation.)
5. The guard itself is a value too — binding, returning, and `drop(g)`
   (which emits `std::move`: drop *consumes*) operate on the wrapper,
   not through it; the adapters must not fire there.

**Enforcement.** `transpiler/tests/guard_context_matrix.rs`: 29
consumption contexts — inline and bound calls, autoderef calls with no
`*` in the source, deref assign/read/compound-assign, reborrowed
arguments, `if let`/`for`/`match`, `lock().unwrap()`, tuple
destructures, user functions returning guards, a fully custom
`impl Deref` guard on no list anywhere, drop-then-reborrow — each
transpiled through the real binary, compiled, and **run**, with the
values asserted. The working rule: a new guard bug means writing the
failing row *first*, then fixing. Row hygiene: rows must be valid Rust
(`Mutex::lock()` returns a `Result`, so `.lock().m()` tests nothing).
...
numeric, `T*` tuples, `C*` const-ness, `R*` routing, `S*` strings,
`G*` guards/autoderef. Each entry follows the same shape:
...
### 7.14 Class G1 — invisible autoderef through guards

The class behind user issues #32–#35, closed systematically in §6.12 /
`docs/guard_autoderef.md`; this entry is the catalogue form.

- **Symptom** — a method call, deref-assign, or free-helper use lands
  on a *guard object* (`RefMut`, `MutexGuard`, a custom `impl Deref`
  type) instead of the value it guards; or a guard prvalue is bound to
  `auto&`. Compile-fail in the lucky cases; **silent-wrong** in the
  unlucky ones (a permissive helper tail answered `false` for a
  guard-wrapped non-empty container; `drop(g)` destroyed a *copy* and
  released no borrow).
...
- **Example**
  ...
  g.push_back(1);              // no `*` anywhere in the source
  ```

  ```cpp
  // WRONG — the model said borrow_mut() yields &mut T
  auto& g = h.q.borrow_mut();  // binds lvalue ref to a temporary
  g.push_back(1);              // no member on rusty::RefMut<...>
  // RIGHT — tolerant either way
  auto&& g = h.q.borrow_mut();
  rusty::deref_call(g, rusty::detail::__mdisp_push_back{}, 1);
  ```

- **Root cause** — Rust's autoderef makes the deref *implicit* (there
  is nothing in the source to translate), and the C++ runtime returns
  guards **by value** where Rust's types read as references; every
  consumption context that trusts the reference reading drops a deref
  that was never written.

- **Fix locus** — `transpiler/src/codegen/guard_flow.rs` (the one
  classifier, both tiers) plus the consumption adapters of §6.11/§6.12;
  for helper-argument leaks, the deref-peel arm of the runtime helper
  in `include/rusty/`. Never a one-off predicate at the emission site —
  that is how the class kept reopening.

- **Recognise it by** — "no member named 'X' in 'rusty::RefMut<…>'"
  (or `MutexGuard`/`Ref`), "cannot bind to a temporary of type
  'RefMut<…>'", "did you mean to use '->' instead of '.'?", or a
  helper returning an impossible answer for a guard-wrapped receiver.
  **Procedure:** add the failing row to
  `transpiler/tests/guard_context_matrix.rs` *first* (it transpiles,
  compiles, and runs every known context), then extend the classifier
  flow edge or the helper peel arm — not the call site.
