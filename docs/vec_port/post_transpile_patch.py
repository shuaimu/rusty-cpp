...
def patch_trim_cmakelists(cpp_out: Path) -> int:
    ...
    # `is_zero` is also dropped: it's a trait-impl helper used by
    # `spec_from_elem` (not in core build). Has its own structural
    # issues (free-fn emit + `this->` body) we defer fixing.
    reduced = """# Auto-generated by rusty-cpp-transpiler, then trimmed by
# docs/vec_port/post_transpile_patch.py.
# REDUCED-SCOPE BUILD — core 6 modules only. Auxiliary modules
# (cow, extract_if, in_place_*, peek_mut, splice, spec_*, partial_eq,
# is_zero) are not built until their cluster-specific issues are
# addressed (see rusty-std-book Chapter 4).
