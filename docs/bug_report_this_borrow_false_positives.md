...
4. ✅ **FIXED: "Cannot borrow from '_temp_call_...': variable is not alive in current scope"**
   - The checker creates temporary variable names with `_temp_call_` and `_temp_expr_` prefixes
...
6. **"Use after move: cannot pass variable 'X' because it has been moved"** (NEW - 2026-01-04)
   ...
   - The checker incorrectly treats clone() as consuming/moving the source
   - Affected file: `src/rrr/reactor/reactor.cc` (recycle method)
