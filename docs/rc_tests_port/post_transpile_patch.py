#!/usr/bin/env python3
"""Post-transpile patches for rc_tests_port.

Currently the transpiled output of `library/alloctests/tests/rc.rs`
...
`transpiled/rc_tests_port/rc_tests_port.cppm` is a hand-stub
...
To regenerate the stub:
    python3 docs/_gen_test_stub.py \
        ~/.rustup/.../library/alloctests/tests/rc.rs \
        rc_tests_port \
        transpiled/rc_tests_port/rc_tests_port.cppm

To switch to a fully-transpiled cppm, transpile + run this script:
    bash docs/rc_tests_port/prep.sh <tgt>/src/lib.rs
