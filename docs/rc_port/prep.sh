...
#   mkdir -p /tmp/rc_port/binary_heap_crate/src
#   cp $RUSTSRC/mod.rs /tmp/rc_port/binary_heap_crate/src/lib.rs
#   cp docs/rc_port/Cargo.toml.template /tmp/rc_port/binary_heap_crate/Cargo.toml
#   bash docs/rc_port/prep.sh /tmp/rc_port/binary_heap_crate/src/lib.rs
set -euo pipefail
...
sed -i \
  ...
  "$SRC"
echo "[rc_port prep] normalized $SRC"
