...
import hashbrown_port.external_trait_impls;

import hashbrown_port.map;

import hashbrown_port.raw_entry;

import hashbrown_port.rustc_entry;
...
export using hasher::DefaultHasher;
...
namespace hash_map {
    ...
    namespace rayon {
        ...
        using namespace ::external_trait_impls::rayon::map;
...
namespace hash_set {
    ...
    namespace rayon {
        ...
        using namespace ::external_trait_impls::rayon::set;
...
namespace hash_table {
    ...
    namespace rayon {
        ...
        using namespace ::external_trait_impls::rayon::table;
...
bool equivalent(const K& key) const {
    return PartialEq::eq((*this), key.borrow());
}
...
// Methods for Q
bool equivalent(const K& key) const {
    return rusty::detail::deref_if_pointer_like((*this)) == key.borrow();
}


rusty::fmt::Result TryReserveError::fmt(rusty::fmt::Formatter& f) const {
    RUSTY_TRY(f.write_str("memory allocation failed"));
    const auto reason = [&]() -> std::string_view { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 0) { return std::string_view(" because the computed capacity exceeded the collection's maximum"); } if (_m.index() == 1) { return std::string_view(" because the memory allocator returned an error"); } return [&]() -> std::string_view { rusty::intrinsics::unreachable(); }(); }();
...
namespace rusty_ext {
    ...
    bool equivalent(const Q& self_, const K& key) {
        ...
        return PartialEq::eq(self_, rusty::deref_call(key, [&](auto&& __recv) -> decltype(std::forward<decltype(__recv)>(__recv).borrow()) { return std::forward<decltype(__recv)>(__recv).borrow(); }));
    }
