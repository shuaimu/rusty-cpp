...
export module hashbrown_port.map;
...
// Rust-only unresolved import: using DefaultHashBuilder;
// Rust-only unresolved import: using Equivalent;
...
return f(std::move(acc), std::make_tuple(std::move(k), std::move(v))); }(); });
    ...
    template<typename T>
    Iter<T> clone() const {
        return Iter<T>{.inner = rusty::clone(this->inner), .marker = rusty::PhantomData<std::tuple<const T&, const V&>>{}};
    }
    template<typename T>
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return f.debug_list().entries(rusty::clone((*this))).finish();
...
return f(std::move(acc), std::make_tuple(std::move(k), std::move(v))); }(); });
    ...
    Iter<K, V> rustc_iter() const {
        return rusty::iter((*this));
    }
    template<typename T>
    Iter<T> iter() const {
        return Iter<T>{.inner = rusty::clone(this->inner), .marker = rusty::PhantomData<std::tuple<const T&, const V&>>{}};
...
struct IntoIter {
    ...
    Iter<K, V> iter() const {
        return Iter<K, V>{.inner = rusty::iter(this->inner), .marker = rusty::PhantomData<std::tuple<const K&, const V&>>{}};
    }
    ...
    template<typename T>
        requires (rusty::alloc::Allocator<A>)
    Iter<T> iter() const {
        return Iter<T>{.inner = rusty::iter(this->inner), .marker = rusty::PhantomData<std::tuple<const T&, const V&>>{}};
    }
    template<typename T>
    static IntoIter<K, V, A> default_() {
        return IntoIter<K, V, A>{.inner = rusty::default_value<raw::RawIntoIter<std::tuple<K, V>, A>>()};
...
struct Keys {
    ...
    rusty::Option<const K&> next() {
        return [&]() -> rusty::Option<const K&> { auto&& _m = this->inner.next(); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<const K&>(k); } if (_m.is_none()) { return rusty::Option<const K&>{rusty::None}; } return [&]() -> rusty::Option<const K&> { rusty::intrinsics::unreachable(); }(); }();
    }
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
        return this->inner.size_hint();
    }
...
struct Values {
    ...
    rusty::Option<const V&> next() {
        return [&]() -> rusty::Option<const V&> { auto&& _m = this->inner.next(); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<const V&>(v); } if (_m.is_none()) { return rusty::Option<const V&>{rusty::None}; } return [&]() -> rusty::Option<const V&> { rusty::intrinsics::unreachable(); }(); }();
    }
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
        return this->inner.size_hint();
    }
...
struct ValuesMut {
    ...
    rusty::Option<V&> next() {
        return [&]() -> rusty::Option<V&> { auto&& _m = this->inner.next(); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<V&>(v); } if (_m.is_none()) { return rusty::Option<V&>{rusty::None}; } return [&]() -> rusty::Option<V&> { rusty::intrinsics::unreachable(); }(); }();
    }
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
        return this->inner.size_hint();
    }
...
/// ```
export template<typename K, typename V, typename S = DefaultHashBuilder, typename A = rusty::alloc::Global>
    requires (rusty::alloc::Allocator<A>)
struct HashMap {
    using Item = std::tuple<K, V>;
    ...
    static HashMap<K, V, S, A> with_capacity(size_t capacity) {
        return HashMap<K, V, S, A>::with_capacity_and_hasher(std::move(capacity), DefaultHashBuilder::default_());
    }
    static HashMap<K, V, S, A> new_in(A alloc) {
        return HashMap<K, V, S, A>::with_hasher_in(DefaultHashBuilder::default_(), std::move(alloc));
    }
    static HashMap<K, V, S, A> with_capacity_in(size_t capacity, A alloc) {
        return HashMap<K, V, S, A>::with_capacity_and_hasher_in(std::move(capacity), DefaultHashBuilder::default_(), std::move(alloc));
    }
    static HashMap<K, V, S, A> with_hasher(S hash_builder) {
    ...
    Iter<K, V> iter() const {
        // @unsafe
        {
            return Iter<K, V>{.inner = rusty::iter(this->table), .marker = rusty::PhantomData<std::tuple<const K&, const V&>>{}};
        }
    }
    IterMut<K, V> iter_mut() {
        // @unsafe
        {
            return IterMut<K, V>{.inner = rusty::iter(this->table), .marker = rusty::PhantomData<std::tuple<const K&, V&>>{}};
        }
    }
    size_t raw_capacity() const {
        return this->table.num_buckets();
    }
    size_t len() const {
        return rusty::len(this->table);
    }
    bool is_empty() const {
    ...
    void retain(F f) {
        // @unsafe
        {
            for (auto&& item : rusty::for_in(rusty::iter(this->table))) {
    ...
    ExtractIf<K, V, F, A> extract_if(F f) {
        return ExtractIf<K, V, F, A>{.f = std::move(f), .inner = raw::RawExtractIf<std::tuple<K, V>, A>{.iter = rusty::iter(this->table), .table = &this->table}};
    }
    void clear() {
        this->table.clear();
    }
    ...
    void reserve(size_t additional) {
        this->table.reserve(std::move(additional), ::make_hasher<auto, V, S>(this->hash_builder));
    }
    auto try_reserve(size_t additional) -> rusty::Result<rusty::Unit, TryReserveError> {
        return this->table.try_reserve(std::move(additional), ::make_hasher<auto, V, S>(this->hash_builder));
    }
    void shrink_to_fit() {
        this->table.shrink_to(static_cast<size_t>(0), ::make_hasher<auto, V, S>(this->hash_builder));
    }
    void shrink_to(size_t min_capacity) {
        this->table.shrink_to(std::move(min_capacity), ::make_hasher<auto, V, S>(this->hash_builder));
    }
    Entry<K, V, S, A> entry(K key) {
        ...
        if (auto&& _iflet_scrutinee = this->table.find(std::move(hash), ::equivalent_key(key)); _iflet_scrutinee.is_some()) {
            ...
            return Entry<K, V, S, A>{Entry_Occupied<K, V, S, A>{OccupiedEntry<K, V, S, A>(std::move(hash), std::move(elem), (*this))}};
        } else {
            return Entry<K, V, S, A>{Entry_Vacant<K, V, S, A>{VacantEntry<K, V, S, A>(std::move(hash), std::move(key), (*this))}};
        }
    }
    template<typename Q>
        requires (rusty::alloc::Allocator<A>)
    EntryRef<K, Q, V, S, A> entry_ref(const Q& key) {
        ...
        if (auto&& _iflet_scrutinee = this->table.find(std::move(hash), ::equivalent_key(key)); _iflet_scrutinee.is_some()) {
            decltype(auto) elem = _iflet_scrutinee.unwrap();
            return EntryRef<K, Q, V, S, A>{EntryRef_Occupied<K, Q, V, S, A>{OccupiedEntry<K, V, S, A>(std::move(hash), std::move(elem), (*this))}};
        } else {
    ...
    rusty::Option<const V&> get(const Q& k) const {
        if (rusty::is_empty(this->table)) {
            ...
            return [&]() -> rusty::Option<const V&> { auto&& _m = this->table.get(hash, ::equivalent_key(k)); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<const V&>(v); } if (_m.is_none()) { return rusty::Option<const V&>{rusty::None}; } return [&]() -> rusty::Option<const V&> { rusty::intrinsics::unreachable(); }(); }();
        }
    }
    template<typename Q>
        requires (rusty::alloc::Allocator<A>)
    ...
    rusty::Option<std::tuple<const K&, V&>> get_key_value_mut(const Q& k) {
        if (rusty::is_empty(this->table)) {
            ...
            const auto hash = ::make_hash<Q, S>(this->hash_builder, k);
            return [&]() -> rusty::Option<std::tuple<const K&, V&>> { auto&& _m = this->table.get_mut(hash, ::equivalent_key(k)); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); const auto& key = std::get<0>(rusty::detail::deref_if_pointer(rusty::detail::deref_if_pointer(_mv0))); auto& value = std::get<1>(rusty::detail::deref_if_pointer(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<std::tuple<const K&, V&>>(std::tuple<const K&, V&>{key, value}); } if (_m.is_none()) { return rusty::Option<std::tuple<const K&, V&>>{rusty::None}; } return [&]() -> rusty::Option<std::tuple<const K&, V&>> { rusty::intrinsics::unreachable(); }(); }();
        }
    }
    template<typename Q>
        requires (rusty::alloc::Allocator<A>)
    bool contains_key(const Q& k) const {
        if (rusty::is_empty(this->table)) {
            ...
            const auto hash = ::make_hash<Q, S>(this->hash_builder, k);
            return this->table.get(hash, ::equivalent_key(k)).is_some();
        }
    }
    template<typename Q>
        requires (rusty::alloc::Allocator<A>)
    rusty::Option<V&> get_mut(const Q& k) {
        if (rusty::is_empty(this->table)) {
            ...
            const auto hash = ::make_hash<Q, S>(this->hash_builder, k);
            return [&]() -> rusty::Option<V&> { auto&& _m = this->table.get_mut(hash, ::equivalent_key(k)); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto& v = std::get<1>(rusty::detail::deref_if_pointer(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<V&>(v); } if (_m.is_none()) { return rusty::Option<V&>{rusty::None}; } return [&]() -> rusty::Option<V&> { rusty::intrinsics::unreachable(); }(); }();
        }
    }
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<V&>, rusty::sanitize_array_capacity<N>()> get_disjoint_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        return rusty::map(this->get_disjoint_mut_inner(ks), [&](auto&& res) -> rusty::Option<V&> { return res.map([&](auto&& _destruct_param0) {
auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_destruct_param0)));
return std::move(v);
}); });
    }
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<V&>, rusty::sanitize_array_capacity<N>()> get_many_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        return this->get_disjoint_mut(ks);
    }
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<V&>, rusty::sanitize_array_capacity<N>()> get_disjoint_unchecked_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        // @unsafe
        {
            return rusty::map(this->get_disjoint_unchecked_mut_inner(ks), [&](auto&& res) -> rusty::Option<V&> { return res.map([&](auto&& _destruct_param0) {
auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_destruct_param0)));
return std::move(v);
}); });
        }
    ...
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<V&>, rusty::sanitize_array_capacity<N>()> get_many_unchecked_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        // @unsafe
        {
    ...
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<std::tuple<const K&, V&>>, rusty::sanitize_array_capacity<N>()> get_disjoint_key_value_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
...
return std::tuple<const K&, V&>{rusty::detail::deref_if_pointer_like(k), v};
    ...
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<std::tuple<const K&, V&>>, rusty::sanitize_array_capacity<N>()> get_many_key_value_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        return this->get_disjoint_key_value_mut(ks);
    }
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<std::tuple<const K&, V&>>, rusty::sanitize_array_capacity<N>()> get_disjoint_key_value_unchecked_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        // @unsafe
        {
...
return std::tuple<const K&, V&>{rusty::detail::deref_if_pointer_like(k), v};
    ...
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<std::tuple<const K&, V&>>, rusty::sanitize_array_capacity<N>()> get_many_key_value_unchecked_mut(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        // @unsafe
        {
    ...
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<std::tuple<K, V>&>, rusty::sanitize_array_capacity<N>()> get_disjoint_mut_inner(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        auto hashes = this->build_hashes_inner(ks);
        return this->table.get_disjoint_mut(hashes, [&](auto&& i, auto&& _destruct_param1) {
auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_destruct_param1)));
return ([&](auto&& __self) -> decltype(auto) { if constexpr (requires { rusty_ext::equivalent(std::forward<decltype(__self)>(__self), std::move(k)); }) { return rusty_ext::equivalent(std::forward<decltype(__self)>(__self), std::move(k)); } else { return rusty_ext::equivalent(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), rusty::detail::deref_if_pointer_like(std::move(k))); } })(ks.at(i));
});
    }
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<rusty::Option<std::tuple<K, V>&>, rusty::sanitize_array_capacity<N>()> get_disjoint_unchecked_mut_inner(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) {
        // @unsafe
        {
            auto hashes = this->build_hashes_inner(ks);
            return this->table.get_disjoint_unchecked_mut(hashes, [&](auto&& i, auto&& _destruct_param1) {
auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_destruct_param1)));
return ([&](auto&& __self) -> decltype(auto) { if constexpr (requires { rusty_ext::equivalent(std::forward<decltype(__self)>(__self), std::move(k)); }) { return rusty_ext::equivalent(std::forward<decltype(__self)>(__self), std::move(k)); } else { return rusty_ext::equivalent(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), rusty::detail::deref_if_pointer_like(std::move(k))); } })(ks.at(i));
});
        }
    ...
    template<typename Q, size_t N>
        requires (rusty::alloc::Allocator<A>)
    std::array<uint64_t, rusty::sanitize_array_capacity<N>()> build_hashes_inner(std::array<std::reference_wrapper<std::add_const_t<Q>>, rusty::sanitize_array_capacity<N>()> ks) const {
...
return rusty::Option<V>{rusty::None}; }(); } return [&]() -> rusty::Option<V> { rusty::intrinsics::unreachable(); }(); }();
    ...
    std::tuple<const K&, V&> insert_unique_unchecked(K k, V v) {
        ...
        const auto bucket = this->table.insert(std::move(hash), std::make_tuple(std::move(k), std::move(v)), ::make_hasher<auto, V, S>(this->hash_builder));
    ...
    rusty::Result<V&, OccupiedError<K, V, S, A>> try_insert(K key, V value) {
        return [&]() -> rusty::Result<V&, OccupiedError<K, V, S, A>> { auto&& _m = this->entry(std::move(key)); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return rusty::Result<V&, OccupiedError<K, V, S, A>>::Err(OccupiedError<K, V, S, A>(std::move(entry), std::move(value))); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return rusty::Result<V&, OccupiedError<K, V, S, A>>::Ok([&]() -> auto& { auto _result_ref_value = (entry.insert(std::move(value))); thread_local std::optional<V> _result_ref_tmp; _result_ref_tmp.reset(); _result_ref_tmp.emplace(std::move(_result_ref_value)); return *_result_ref_tmp; }()); } return [&]() -> rusty::Result<V&, OccupiedError<K, V, S, A>> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename Q>
        requires (rusty::alloc::Allocator<A>)
    rusty::Option<V> remove(const Q& k) {
        return [&]() -> rusty::Option<V> { auto&& _m = this->remove_entry(k); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<V>(std::move(v)); } if (_m.is_none()) { return rusty::Option<V>{rusty::None}; } return [&]() -> rusty::Option<V> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename Q>
        requires (rusty::alloc::Allocator<A>)
    rusty::Option<std::tuple<K, V>> remove_entry(const Q& k) {
        const auto hash = ::make_hash<Q, S>(this->hash_builder, k);
        return this->table.remove_entry(hash, ::equivalent_key(k));
    }
    size_t allocation_size() const {
        return this->table.allocation_size();
    }
    bool operator==(const HashMap<K, V, S, A>& other) const {
        if (rusty::len((*this)) != rusty::len(other)) {
            return false;
        }
        return rusty::iter((*this)).all([&](auto&& _destruct_param0) {
...
return other.get(rusty::detail::deref_if_pointer_like(key)).is_some_and([&](auto&& v) { return rusty::detail::deref_if_pointer_like(value) == rusty::deref_mut(v); });
});
    }
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return f.debug_map().entries(rusty::iter((*this))).finish();
    }
    ...
    template<size_t N>
        requires (rusty::alloc::Allocator<A>)
    static HashMap<K, V, S, A> from(std::array<std::tuple<K, V>, rusty::sanitize_array_capacity<N>()> arr) {
    ...
    static HashMap<K, V, S, A> from_iter(T iter) {
        ...
        auto map = HashMap<K, V, S, A>::with_capacity_and_hasher_in(([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(iter_shadow1.size_hint()), S::default_(), A::default_());
        iter_shadow1.for_each([&](auto&& _destruct_param0) {
auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_destruct_param0)));
auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_destruct_param0)));
map.insert(std::move(k), std::move(v));
    ...
    void extend(T iter) {
        auto iter_shadow1 = rusty::iter(std::move(iter));
        auto reserve = (rusty::is_empty((*this)) ? ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(iter_shadow1.size_hint()) : ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(iter_shadow1.size_hint()).div_ceil(2));
        this->reserve(std::move(reserve));
        iter_shadow1.for_each([=](auto&& _destruct_param0) mutable {
auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_destruct_param0)));
auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_destruct_param0)));
this->insert(std::move(k), std::move(v));
    ...
    void extend_reserve(size_t additional) {
        auto reserve = (rusty::is_empty((*this)) ? additional : additional.div_ceil(2));
        this->reserve(std::move(reserve));
    }
    void extend_one(std::tuple<const K&, const V&> _arg1) {
        auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_arg1)));
        auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_arg1)));
        this->insert(std::move(rusty::detail::deref_if_pointer_like(k)), std::move(rusty::detail::deref_if_pointer_like(v)));
    }
    void extend_one(const std::tuple<K, V>& _arg1) {
        auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(rusty::detail::deref_if_pointer(_arg1))));
        auto&& v = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(rusty::detail::deref_if_pointer(_arg1))));
        this->insert(std::move(k), std::move(v));
    }
    RawEntryBuilderMut<K, V, S, A> raw_entry_mut() {
        return RawEntryBuilderMut<K, V, S, A>{.map = (*this)};
    }
    RawEntryBuilder<K, V, S, A> raw_entry() const {
        return RawEntryBuilder<K, V, S, A>{.map = (*this)};
    }
    RustcEntry<K, V, A> rustc_entry(K key) {
        auto hash = ::make_hash(this->hash_builder, key);
        if (auto&& _iflet_scrutinee = this->table.find(std::move(hash), [&](auto&& q) { return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(q).eq(key); }); _iflet_scrutinee.is_some()) {
            decltype(auto) elem = _iflet_scrutinee.unwrap();
            return RustcEntry<K, V, A>{RustcEntry_Occupied<K, V, A>{RustcOccupiedEntry<K, V, A>{.elem = std::move(elem), .table = &this->table}}};
        } else {
            this->reserve(static_cast<size_t>(1));
            return RustcEntry<K, V, A>{RustcEntry_Vacant<K, V, A>{RustcVacantEntry<K, V, A>{.hash = std::move(hash), .key = std::move(key), .table = &this->table}}};
        }
    }
};
...
struct Entry : std::variant<Entry_Occupied<K, V, S, A>, Entry_Vacant<K, V, S, A>> {
    ...
    OccupiedEntry<K, V, S, A> insert(V value) {
        return [&]() -> OccupiedEntry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> OccupiedEntry<K, V, S, A> { entry.insert(std::move(value));
return entry; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry(std::move(value)); } return [&]() -> OccupiedEntry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    ...
    OccupiedEntry<K, V, S, A> or_insert_entry(V default_) {
        return [&]() -> OccupiedEntry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry(std::move(default_)); } return [&]() -> OccupiedEntry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename F>
...
return entry.insert(std::move(value)); }(); } return [&]() -> V& { rusty::intrinsics::unreachable(); }(); }();
    ...
    Entry<K, V, S, A> and_modify(F f) {
        return [&]() -> Entry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> Entry<K, V, S, A> { f(entry.get_mut());
return Entry<K, V, S, A>{Entry_Occupied<K, V, S, A>{entry}}; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return Entry<K, V, S, A>{Entry_Vacant<K, V, S, A>{entry}}; } return [&]() -> Entry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename F>
    Entry<K, V, S, A> and_replace_entry_with(F f) {
        return [&]() -> Entry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.replace_entry_with(std::move(f)); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { return this->clone(); } return [&]() -> Entry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    rusty::HashMap<K, V, S, A>& into_map() {
        return [&]() -> rusty::HashMap<K, V, S, A>& { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.table; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.table; } return [&]() -> rusty::HashMap<K, V, S, A>& { rusty::intrinsics::unreachable(); }(); }();
    }
    V& or_default() {
        return [&]() -> V& { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.into_mut(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert(V::default_()); } return [&]() -> V& { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename T>
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return [&]() -> rusty::fmt::Result { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 0) { const auto& v = std::get<0>(rusty::detail::deref_if_pointer(_m))._0; return f.debug_tuple("Entry").field(v).finish(); } if (rusty::detail::deref_if_pointer(_m).index() == 1) { const auto& o = std::get<1>(rusty::detail::deref_if_pointer(_m))._0; return f.debug_tuple("Entry").field(o).finish(); } return [&]() -> rusty::fmt::Result { rusty::intrinsics::unreachable(); }(); }();
    ...
    OccupiedEntry<T, S, A> insert() {
        return [&]() -> OccupiedEntry<T, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert(); } return [&]() -> OccupiedEntry<T, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename T>
    void or_insert() {
...
return entry; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert(std::move(value)); } return [&]() -> OccupiedEntry<T, A> { rusty::intrinsics::unreachable(); }(); }();
    ...
    template<typename T>
        requires (rusty::alloc::Allocator<A>)
    Entry<K, V, S, A> and_modify(const auto& f) {
        return [&]() -> Entry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> Entry<K, V, S, A> { f(entry.get_mut());
return Entry<K, V, S, A>{Entry_Occupied<K, V, S, A>{entry}}; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return Entry<K, V, S, A>{Entry_Vacant<K, V, S, A>{entry}}; } return [&]() -> Entry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename T>
...
struct OccupiedEntry {
    ...
    K replace_key(K key) {
        if (!(([&](auto&& __self) -> decltype(auto) { if constexpr (requires { rusty_ext::equivalent(std::forward<decltype(__self)>(__self), key); }) { return rusty_ext::equivalent(std::forward<decltype(__self)>(__self), key); } else { return rusty_ext::equivalent(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), rusty::detail::deref_if_pointer_like(key)); } })(this->key()))) { throw std::logic_error("replaced key is not equivalent to the one in the entry"); }
        // @unsafe
        {
            return this->replace_key_unchecked(std::move(key));
        }
    }
    K replace_key_unchecked(K key) {
        return rusty::mem::replace(rusty::addr_of_temp(([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(this->elem.as_mut())), std::move(key));
    }
    ...
    std::tuple<const K&, V&> into_entry() {
        auto [key, val] = rusty::detail::deref_if_pointer_like(this->elem.as_mut());
...
if (auto&& _iflet_scrutinee = f(key, std::move(value)); _iflet_scrutinee.is_some()) {
    ...
    return rusty::None;
            ...
            if (spare_key.is_some()) {
                ...
                return Entry<K, V, S, A>{Entry_Vacant<K, V, S, A>{VacantEntry<K, V, S, A>(std::move(this->hash), std::move(key), this->table)}};
            } else {
    ...
    Entry<T, A> replace_entry_with(F f) {
        // @unsafe
        {
            return [&]() -> Entry<T, A> { auto&& _m = this->table.raw.replace_bucket_with(rusty::clone(this->bucket), std::move(f)); if (_m.is_none()) { return Entry<T, A>{Entry_Occupied<T, A>{std::move((*this))}}; } if (_m.is_some()) { auto&& _mv1 = _m.unwrap(); auto&& tag = rusty::detail::deref_if_pointer(_mv1); return Entry<T, A>{Entry_Vacant<T, A>{VacantEntry<T, A, S, A>{.table = this->table, .tag = std::move(tag), .index = this->bucket_index()}}}; } return [&]() -> Entry<T, A> { rusty::intrinsics::unreachable(); }(); }();
        }
    }
};
...
struct VacantEntry {
    ...
    V& insert(V value) {
        ...
        const auto entry = table.insert_entry(std::move(this->hash), std::make_tuple(std::move(this->key_field), std::move(value)), ::make_hasher<auto, V, S>(this->table.hash_builder));
    ...
    OccupiedEntry<K, V, S, A> insert_entry(V value) {
        auto elem = this->table.table.insert(std::move(this->hash), std::make_tuple(std::move(this->key_field), std::move(value)), ::make_hasher<auto, V, S>(this->table.hash_builder));
    ...
    OccupiedEntry<T, A> insert(T value) {
        ...
        return OccupiedEntry<T, A>{.table = this->table, .bucket = std::move(bucket)};
    }
    template<typename T>
        requires (rusty::alloc::Allocator<A>)
    HashTable<T, A>& into_table() {
...
struct EntryRef : std::variant<EntryRef_Occupied<K, Q, V, S, A>, EntryRef_Vacant<K, Q, V, S, A>> {
    ...
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return [&]() -> rusty::fmt::Result { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 0) { const auto& v = std::get<0>(rusty::detail::deref_if_pointer(_m))._0; return f.debug_tuple("EntryRef").field(v).finish(); } if (rusty::detail::deref_if_pointer(_m).index() == 1) { const auto& o = std::get<1>(rusty::detail::deref_if_pointer(_m))._0; return f.debug_tuple("EntryRef").field(o).finish(); } return [&]() -> rusty::fmt::Result { rusty::intrinsics::unreachable(); }(); }();
    }
    OccupiedEntry<K, V, S, A> insert(V value) {
        return [&]() -> OccupiedEntry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> OccupiedEntry<K, V, S, A> { entry.insert(std::move(value));
return entry; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry(std::move(value)); } return [&]() -> OccupiedEntry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    ...
    template<typename F>
    V& or_insert_with_key(F default_) {
        return [&]() -> V& { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.into_mut(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> V& { auto value = default_(std::move(entry.key));
return entry.insert(std::move(value)); }(); } return [&]() -> V& { rusty::intrinsics::unreachable(); }(); }();
    }
    const Q& key() const {
        return [&]() -> const Q& { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { const auto& entry = std::get<1>(rusty::detail::deref_if_pointer(_m))._0; return entry.key().borrow(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { const auto& entry = std::get<0>(rusty::detail::deref_if_pointer(_m))._0; return entry.key(); } return [&]() -> const Q& { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename F>
    EntryRef<K, Q, V, S, A> and_modify(F f) {
        return [&]() -> EntryRef<K, Q, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> EntryRef<K, Q, V, S, A> { f(entry.get_mut());
return EntryRef<K, Q, V, S, A>{EntryRef_Occupied<K, Q, V, S, A>{entry}}; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return EntryRef<K, Q, V, S, A>{EntryRef_Vacant<K, Q, V, S, A>{entry}}; } return [&]() -> EntryRef<K, Q, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    rusty::HashMap<K, V, S, A>& into_map() {
        return [&]() -> rusty::HashMap<K, V, S, A>& { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.table; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.table; } return [&]() -> rusty::HashMap<K, V, S, A>& { rusty::intrinsics::unreachable(); }(); }();
    }
    ...
    OccupiedEntry<K, V, S, A> or_default_entry() {
        return [&]() -> OccupiedEntry<K, V, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry(rusty::default_value<V>()); } return [&]() -> OccupiedEntry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
};
template<typename K, typename Q, typename V, typename S, typename A>
...
/// assert!(map["b"] == 20 && map.len() == 2);
/// ```
export template<typename K, typename Q, typename V, typename S, typename A = rusty::alloc::Global>
    requires (rusty::alloc::Allocator<A>)
struct VacantEntryRef {
    uint64_t hash;
    const Q& key_field;
    ...
    V& insert(V value) {
        auto& table = this->table.table;
        const auto entry = table.insert_entry(std::move(this->hash), std::make_tuple(rusty::to_owned(this->key_field), std::move(value)), ::make_hasher<auto, V, S>(this->table.hash_builder));
        return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._1; }) return (std::forward<decltype(__t)>(__t)._1); else return std::get<1>(std::forward<decltype(__t)>(__t)); })(entry);
    }
    V& insert_with_key(K key, V value) {
        return this->insert_entry_with_key(std::move(key), std::move(value)).into_mut();
    }
    V& insert_with_key_unchecked(K key, V value) {
        return this->insert_entry_with_key_unchecked(std::move(key), std::move(value)).into_mut();
    }
    OccupiedEntry<K, V, S, A> insert_entry(V value) {
        auto elem = this->table.table.insert(std::move(this->hash), std::make_tuple(rusty::to_owned(this->key_field), std::move(value)), ::make_hasher<auto, V, S>(this->table.hash_builder));
        return OccupiedEntry<K, V, S, A>{.hash = std::move(this->hash), .elem = std::move(elem), .table = this->table};
    }
    OccupiedEntry<K, V, S, A> insert_entry_with_key(K key, V value) {
        if (!(([&](auto&& __self) -> decltype(auto) { if constexpr (requires { rusty_ext::equivalent(std::forward<decltype(__self)>(__self), key); }) { return rusty_ext::equivalent(std::forward<decltype(__self)>(__self), key); } else { return rusty_ext::equivalent(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), rusty::detail::deref_if_pointer_like(key)); } })((this->key_field)))) { throw std::logic_error("key used for Entry creation is not equivalent to the one used for insertion"); }
        // @unsafe
        {
            return this->insert_entry_with_key_unchecked(std::move(key), std::move(value));
        }
    }
    OccupiedEntry<K, V, S, A> insert_entry_with_key_unchecked(K key, V value) {
        auto elem = this->table.table.insert(std::move(this->hash), std::make_tuple(std::move(key), std::move(value)), ::make_hasher<auto, V, S>(this->table.hash_builder));
        return OccupiedEntry<K, V, S, A>{.hash = std::move(this->hash), .elem = std::move(elem), .table = this->table};
    }
    rusty::HashMap<K, V, S, A>& into_map() {
        return this->table;
...
/// assert_eq!(map[&"a"], 100);
/// ```
export template<typename K, typename V, typename S, typename A = rusty::alloc::Global>
    requires (rusty::alloc::Allocator<A>)
struct OccupiedError {
...
const auto& equivalent_key(const Q& k) {
    return [=, k = std::move(k)](auto&& x) mutable -> bool { return ([&](auto&& __self) -> decltype(auto) { if constexpr (requires { rusty_ext::equivalent(std::forward<decltype(__self)>(__self), &([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(x)); }) { return rusty_ext::equivalent(std::forward<decltype(__self)>(__self), &([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(x)); } else { return rusty_ext::equivalent(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), rusty::detail::deref_if_pointer_like(&([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(x))); } })(k); };
}

/// Ensures that a single closure type across uses of this which, in turn prevents multiple
/// instances of any functions like `RawTable::reserve` from being generated
export template<typename Q, typename K>
const auto& equivalent(const Q& k) {
    return [=, k = std::move(k)](auto&& x) mutable -> bool { return ([&](auto&& __self) -> decltype(auto) { if constexpr (requires { rusty_ext::equivalent(std::forward<decltype(__self)>(__self), std::move(x)); }) { return rusty_ext::equivalent(std::forward<decltype(__self)>(__self), std::move(x)); } else { return rusty_ext::equivalent(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), rusty::detail::deref_if_pointer_like(std::move(x))); } })(k); };
}
...
void assert_covariance() {
    const rusty::SafeFn<rusty::HashMap<std::string_view, uint8_t>(rusty::HashMap<std::string_view, uint8_t>)> map_key = +[](rusty::HashMap<std::string_view, uint8_t> v) -> rusty::HashMap<std::string_view, uint8_t> {
        return std::move(v);
    };
    const rusty::SafeFn<rusty::HashMap<uint8_t, std::string_view>(rusty::HashMap<uint8_t, std::string_view>)> map_val = +[](rusty::HashMap<uint8_t, std::string_view> v) -> rusty::HashMap<uint8_t, std::string_view> {
        return std::move(v);
    };
