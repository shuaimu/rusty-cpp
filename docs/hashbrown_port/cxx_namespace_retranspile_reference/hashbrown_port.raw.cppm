...
if (_m == Fallibility::Infallible) /* panic!("Hash table capacity overflow") */; }();
}
...
if (_m == Fallibility::Infallible) return handle_alloc_error(std::move(layout)); }();
}

// Rust-only unresolved import: using TryReserveError;
...
struct Bucket {
    ...
    static Bucket<T> from_base_index(rusty::ptr::NonNull<T> base, size_t index) {
        const auto ptr_shadow1 = (T::IS_ZERO_SIZED ? invalid_mut(rusty::detail::deref_if_pointer_like(index) + 1) : rusty::ptr::sub(rusty::as_ptr(base), std::move(index)));
    ...
    std::add_pointer_t<T> as_ptr() const {
        if (T::IS_ZERO_SIZED) {
            return invalid_mut(rusty::mem::align_of<T>());
        } else {
            // @unsafe
            {
    ...
    Bucket<T> next_n(size_t offset) const {
        const auto ptr_shadow1 = (T::IS_ZERO_SIZED ? invalid_mut((static_cast<size_t>(reinterpret_cast<std::uintptr_t>(rusty::as_ptr(this->ptr)))) + rusty::detail::deref_if_pointer_like(offset)) : rusty::ptr::sub(rusty::as_ptr(this->ptr), std::move(offset)));
        return Bucket<T>{.ptr = rusty::ptr::NonNull<T>::new_unchecked(std::move(ptr_shadow1))};
    }
    void drop() const {
        // @unsafe
        {
            rusty::as_ptr((*this))->drop_in_place();
        }
    }
    T read() const {
        // @unsafe
...
struct RawIterRange {
    using Item = Bucket<T>;
    control::BitMaskIter current_group;
    Bucket<T> data;
    ...
    static RawIterRange<T> new_(const uint8_t* ctrl, Bucket<T> data, size_t len) {
        ...
        auto [current_group, next_ctrl] = rusty::detail::deref_if_pointer_like(std::make_tuple(Group::load_aligned(ctrl->cast()).match_full(), rusty::ptr::add(ctrl, rusty::clone(rusty::clone(Group::WIDTH)))));
        return RawIterRange<T>{.current_group = rusty::iter(std::move(current_group)), .data = std::move(data), .next_ctrl = std::move(next_ctrl), .end = end};
    }
    std::tuple<RawIterRange<T>, rusty::Option<RawIterRange<T>>> split() {
        // @unsafe
        {
            if (this->end <= this->next_ctrl) {
                ...
                assert((self . data . next_n (Group :: WIDTH) . next_n (mid) . ptr == tail . data . ptr));
                assert((self . end == tail . end));
                this->end = rusty::ptr::add(this->next_ctrl, std::move(mid));
                assert((self . end . add (Group :: WIDTH) == tail . next_ctrl));
    ...
    rusty::Option<Bucket<T>> next_impl() {
        while (true) {
            ...
            // @unsafe
            {
                this->current_group = rusty::iter(Group::load_aligned(this->next_ctrl->cast()).match_full());
    ...
    B fold_impl(size_t n, B acc, F f) {
        while (true) {
            while (true) {
                ...
                assert((rusty::detail::deref_if_pointer_like(n) != static_cast<size_t>(0)));
...
struct RawIter {
    ...
    void drop_elements() {
        // @unsafe
        {
            if (T::NEEDS_DROP && (rusty::detail::deref_if_pointer_like(this->items) != static_cast<size_t>(0))) {
...
struct RawTableInner {
    ...
    template<typename A>
        requires (rusty::alloc::Allocator<A>)
    static auto new_uninitialized(const A& alloc, const auto& table_layout, size_t buckets, const auto& fallibility) -> rusty::Result<RawTableInner, TryReserveError>;
    template<typename A>
        requires (rusty::alloc::Allocator<A>)
    static auto fallible_with_capacity(const A& alloc, const auto& table_layout, size_t capacity, const auto& fallibility) -> rusty::Result<RawTableInner, TryReserveError>;
    template<typename A>
        requires (rusty::alloc::Allocator<A>)
    ...
    rusty::Option<size_t> find_insert_index_in_group(const control::Group& group, const ProbeSeq& probe_seq) const;
    rusty::Result<size_t, size_t> find_or_find_insert_index_inner(uint64_t hash, const std::function<bool(size_t)>& eq) const;
    std::tuple<size_t, control::Tag> prepare_insert_index(uint64_t hash);
    ...
    void record_item_insert_at(size_t index, control::Tag old_ctrl, control::Tag new_ctrl);
    bool is_in_same_group(size_t i, size_t new_i, uint64_t hash) const;
    void set_ctrl_hash(size_t index, uint64_t hash);
    control::Tag replace_ctrl_hash(size_t index, uint64_t hash);
    void set_ctrl(size_t index, control::Tag ctrl);
    control::Tag* ctrl(size_t index) const;
    std::span<mem::MaybeUninit<control::Tag>> ctrl_slice();
    ...
    template<typename A>
        requires (rusty::alloc::Allocator<A>)
    auto prepare_resize(const A& alloc, const auto& table_layout, size_t capacity, const auto& fallibility) const -> rusty::Result<::scopeguard::ScopeGuard<RawTableInner, std::function<void(RawTableInner&)>>, TryReserveError>;
    template<typename A>
        requires (rusty::alloc::Allocator<A>)
    auto reserve_rehash_inner(const A& alloc, size_t additional, const std::function<uint64_t(RawTableInner&, size_t)>& hasher, const auto& fallibility, const auto& layout, rusty::Option<rusty::UnsafeFn<void(uint8_t*)>> drop) -> rusty::Result<rusty::Unit, TryReserveError>;
    FullBucketsIndices full_buckets_indices() const;
    template<typename A>
        requires (rusty::alloc::Allocator<A>)
    auto resize_inner(const A& alloc, size_t capacity, const std::function<uint64_t(RawTableInner&, size_t)>& hasher, const auto& fallibility, const auto& layout) -> rusty::Result<rusty::Unit, TryReserveError>;
...
export struct RawIterHashIndices {
    ...
    size_t bucket_mask;
    rusty::ptr::NonNull<uint8_t> ctrl;
    control::Tag tag_hash;
    ProbeSeq probe_seq;
    control::Group group;
    control::BitMaskIter bitmask;
...
struct RawTable {
    ...
    static rusty::Result<RawTable<T, A>, TryReserveError> new_uninitialized(A alloc, size_t buckets, const auto& fallibility) {
    ...
    Bucket<T> bucket(size_t index) const {
        assert((self . table . bucket_mask != 0));
        assert((rusty::detail::deref_if_pointer_like(index) < this->num_buckets()));
        // @unsafe
        {
    ...
    std::tuple<T, size_t, control::Tag> remove_tagged(Bucket<T> item) {
        // @unsafe
        {
            auto index = this->bucket_index(item);
    ...
    void shrink_to(size_t min_size, const auto& hasher) {
        auto min_size_shadow1 = usize::max(this->table.items, std::move(min_size));
        if (rusty::detail::deref_if_pointer_like(min_size_shadow1) == 0) {
            ...
            // @unsafe
            {
                old_inner.template drop_inner_table<T, std::remove_cvref_t<decltype((rusty::clone(rusty::clone(RawTable<T, A>::TABLE_LAYOUT))))>>(this->alloc, rusty::clone(rusty::clone(RawTable<T, A>::TABLE_LAYOUT)));
            }
            return;
        }
    ...
    auto try_reserve(size_t additional, const auto& hasher) -> rusty::Result<rusty::Unit, TryReserveError> {
    ...
    auto reserve_rehash(size_t additional, const auto& hasher, const auto& fallibility) -> rusty::Result<rusty::Unit, TryReserveError> {
        // @unsafe
        {
            return this->table.reserve_rehash_inner(this->alloc, std::move(additional), [&](auto&& table, auto&& index) -> uint64_t { return hasher(table.template bucket<T>(std::move(index)).as_ref()); }, std::move(fallibility), rusty::clone(rusty::clone(RawTable<T, A>::TABLE_LAYOUT)), (T::NEEDS_DROP ? rusty::Option<rusty::UnsafeFn<void(uint8_t*)>>([&](auto&& ptr_shadow1) -> rusty::UnsafeFn<void(uint8_t*)> { return rusty::ptr::drop_in_place(ptr_shadow1.template cast<T>()); }) : rusty::Option<rusty::UnsafeFn<void(uint8_t*)>>{rusty::None}));
        }
    }
    auto resize(size_t capacity, const auto& hasher, const auto& fallibility) -> rusty::Result<rusty::Unit, TryReserveError> {
        // @unsafe
        {
    ...
    Bucket<T> insert_no_grow(uint64_t hash, T value) {
        // @unsafe
        {
            ...
            bucket.write(std::move(value));
            this->table.items += 1;
            return std::move(bucket);
        }
    }
    template<typename F>
    rusty::Option<control::Tag> replace_bucket_with(Bucket<T> bucket, F f) {
        // @unsafe
        {
            auto index = this->bucket_index(bucket);
            auto old_ctrl = *this->table.ctrl(std::move(index));
            assert((this->is_bucket_full(std::move(index))));
            ...
            if (auto&& _iflet_scrutinee = f(std::move(item)); _iflet_scrutinee.is_some()) {
                ...
                return rusty::Option<control::Tag>{rusty::None};
            } else {
                return rusty::Option<control::Tag>(std::move(old_ctrl));
            }
        }
    }
    ...
    Bucket<T> insert_tagged_at_index(control::Tag tag, size_t index, T value) {
        // @unsafe
        {
            auto old_ctrl = *this->table.ctrl(std::move(index));
...
return rusty::Some(std::make_tuple(NonNull<std::remove_pointer_t<std::remove_reference_t<decltype((rusty::ptr::sub(rusty::as_ptr(this->table.ctrl_field), std::move(ctrl_offset))->cast()))>>>::new_unchecked(rusty::ptr::sub(rusty::as_ptr(this->table.ctrl_field), std::move(ctrl_offset))->cast()), std::move(layout), rusty::ptr::read(&this->alloc)));
}
}();
    ...
    RawTable<T, A> clone() const {
        if (this->table.is_empty_singleton()) {
            ...
            auto new_table = result.unwrap_unchecked();
            // @unsafe
            {
...
return result.unwrap_unchecked(); }();
                ...
                ScopeGuard::into_inner(std::move(self_));
            }
        }
    }
...
if (T::NEEDS_DROP) {
    ...
    ~RawTable() noexcept(false) {
        ...
        // @unsafe
        {
            this->table.template drop_inner_table<T, std::remove_cvref_t<decltype((rusty::clone(rusty::clone(RawTable<T, A>::TABLE_LAYOUT))))>>(this->alloc, rusty::clone(rusty::clone(RawTable<T, A>::TABLE_LAYOUT)));
        }
    }
...
rusty::Option<size_t> capacity_to_buckets(size_t cap, const auto& table_layout) {
    ...
    auto buckets = adjusted_cap.next_power_of_two();
    ::ensure_bucket_bytes_at_least_ctrl_align(std::move(table_layout), std::move(buckets));
    return rusty::Option<size_t>(std::move(buckets));
}
...
void ensure_bucket_bytes_at_least_ctrl_align(const auto& table_layout, size_t buckets) {
    if (rusty::detail::deref_if_pointer_like(table_layout.size) != static_cast<size_t>(0)) {
        ...
        assert((rusty::detail::deref_if_pointer_like(prod) >= rusty::detail::deref_if_pointer_like(table_layout.ctrl_align)));
    }
}
...
rusty::Option<std::tuple<rusty::alloc::Layout, size_t>> TableLayout::calculate_layout_for(size_t buckets) {
    ...
    auto ctrl_offset = RUSTY_TRY_OPT([&]() { auto&& _checked_lhs = RUSTY_TRY_OPT([&]() { auto&& _checked_lhs = size; return rusty::checked_mul(_checked_lhs, static_cast<std::remove_cvref_t<decltype((_checked_lhs))>>(std::move(buckets))); }()); return rusty::checked_add(_checked_lhs, static_cast<std::remove_cvref_t<decltype((_checked_lhs))>>(rusty::detail::deref_if_pointer_like(ctrl_align) - 1)); }()) & ~(rusty::detail::deref_if_pointer_like(ctrl_align) - static_cast<int32_t>(1));
    const auto len = RUSTY_TRY_OPT([&]() { auto&& _checked_lhs = ctrl_offset; return rusty::checked_add(_checked_lhs, static_cast<std::remove_cvref_t<decltype((_checked_lhs))>>(rusty::detail::deref_if_pointer_like(buckets) + rusty::clone(Group::WIDTH))); }());
...
rusty::Option<size_t> FullBucketsIndices::next_impl() {
    ...
    while (true) {
        ...
        // @unsafe
        {
            this->current_group = rusty::iter(Group::load_aligned(rusty::as_ptr(this->ctrl)->cast()).match_full());
...
RawTableInner RawTableInner::new_() {
    return RawTableInner{.bucket_mask = static_cast<size_t>(0), .ctrl_field = rusty::ptr::NonNull<uint8_t>::new_unchecked(rusty::as_ptr(Group::static_empty())->cast_mut().cast()), .growth_left = static_cast<size_t>(0), .items = static_cast<size_t>(0)};
}

template<typename A>
    requires (rusty::alloc::Allocator<A>)
auto RawTableInner::new_uninitialized(const A& alloc, const auto& table_layout, size_t buckets, const auto& fallibility) -> rusty::Result<RawTableInner, TryReserveError> {
    assert((buckets.is_power_of_two()));
...
_match_value.emplace(std::move([&]() -> rusty::ptr::NonNull<uint8_t> { if (rusty::len(block) != layout.size) {
    ...
    assert((rusty::detail::deref_if_pointer_like(x) >= rusty::detail::deref_if_pointer_like(buckets)));
...
// @unsafe
{
    ...
    assert((oversized_layout.size() <= rusty::len(block)));
    assert((rusty::detail::deref_if_pointer_like(oversized_ctrl_offset) >= rusty::detail::deref_if_pointer_like(ctrl_offset)));
...
template<typename A>
    requires (rusty::alloc::Allocator<A>)
auto RawTableInner::fallible_with_capacity(const A& alloc, const auto& table_layout, size_t capacity, const auto& fallibility) -> rusty::Result<RawTableInner, TryReserveError> {
    if (rusty::detail::deref_if_pointer_like(capacity) == static_cast<size_t>(0)) {
        ...
        // @unsafe
        {
            auto buckets = RUSTY_TRY_INTO(::capacity_to_buckets(std::move(capacity), std::move(table_layout)).ok_or_else([&]() { return capacity_overflow(fallibility); }), rusty::Result<RawTableInner, TryReserveError>);
...
size_t RawTableInner::fix_insert_index(size_t index) const {
    if (unlikely(this->is_bucket_full(std::move(index)))) {
        assert((rusty::detail::deref_if_pointer_like(this->bucket_mask) < rusty::clone(Group::WIDTH)));
        index = Group::load_aligned(this->ctrl(static_cast<size_t>(0))).match_empty_or_deleted().lowest_set_bit().unwrap_unchecked();
    }
    return std::move(index);
}

rusty::Option<size_t> RawTableInner::find_insert_index_in_group(const control::Group& group, const ProbeSeq& probe_seq) const {
...
std::tuple<size_t, control::Tag> RawTableInner::prepare_insert_index(uint64_t hash) {
    // @unsafe
    {
...
void RawTableInner::prepare_rehash_in_place() {
    // @unsafe
    {
        for (auto&& i : rusty::for_in((rusty::range(0, this->num_buckets())).step_by(rusty::clone(rusty::clone(Group::WIDTH))))) {
            const auto group = Group::load_aligned(this->ctrl(std::move(i)));
...
void RawTableInner::record_item_insert_at(size_t index, control::Tag old_ctrl, control::Tag new_ctrl) {
...
control::Tag RawTableInner::replace_ctrl_hash(size_t index, uint64_t hash) {
    // @unsafe
    {
...
void RawTableInner::set_ctrl(size_t index, control::Tag ctrl) {
...
control::Tag* RawTableInner::ctrl(size_t index) const {
    assert((rusty::detail::deref_if_pointer_like(index) < this->num_ctrl_bytes()));
    // @unsafe
    {
        return rusty::ptr::add(rusty::as_ptr(this->ctrl_field), std::move(index))->cast();
    }
}

std::span<mem::MaybeUninit<control::Tag>> RawTableInner::ctrl_slice() {
    // @unsafe
    {
        return rusty::from_raw_parts_mut(rusty::as_ptr(this->ctrl_field)->cast(), this->num_ctrl_bytes());
    }
}
...
template<typename A>
    requires (rusty::alloc::Allocator<A>)
auto RawTableInner::prepare_resize(const A& alloc, const auto& table_layout, size_t capacity, const auto& fallibility) const -> rusty::Result<::scopeguard::ScopeGuard<RawTableInner, std::function<void(RawTableInner&)>>, TryReserveError> {
    assert((rusty::detail::deref_if_pointer_like(this->items) <= rusty::detail::deref_if_pointer_like(capacity)));
    const auto new_table = RUSTY_TRY_INTO(RawTableInner::fallible_with_capacity(alloc, std::move(table_layout), std::move(capacity), std::move(fallibility)), rusty::Result<::scopeguard::ScopeGuard<RawTableInner, std::function<void(RawTableInner&)>>, TryReserveError>);
    return rusty::Result<::scopeguard::ScopeGuard<RawTableInner, std::function<void(RawTableInner&)>>, TryReserveError>::Ok(guard(std::move(new_table), [=, alloc = std::move(alloc), table_layout = std::move(table_layout)](auto&& self_) mutable {
...
template<typename A>
    requires (rusty::alloc::Allocator<A>)
auto RawTableInner::reserve_rehash_inner(const A& alloc, size_t additional, const std::function<uint64_t(RawTableInner&, size_t)>& hasher, const auto& fallibility, const auto& layout, rusty::Option<rusty::UnsafeFn<void(uint8_t*)>> drop) -> rusty::Result<rusty::Unit, TryReserveError> {
    ...
    if (rusty::detail::deref_if_pointer_like(new_items) <= (rusty::detail::deref_if_pointer_like(full_capacity) / 2)) {
        ...
        // @unsafe
        {
            return this->resize_inner(alloc, usize::max(std::move(new_items), rusty::detail::deref_if_pointer_like(full_capacity) + 1), hasher, std::move(fallibility), std::move(layout));
        }
    }
}
...
FullBucketsIndices RawTableInner::full_buckets_indices() const {
    // @unsafe
    {
        ...
        return FullBucketsIndices{.current_group = rusty::iter(Group::load_aligned(rusty::as_ptr(ctrl)->cast()).match_full()), .group_first_index = static_cast<size_t>(0), .ctrl = std::move(ctrl), .items = this->items};
    }
}

template<typename A>
    requires (rusty::alloc::Allocator<A>)
auto RawTableInner::resize_inner(const A& alloc, size_t capacity, const std::function<uint64_t(RawTableInner&, size_t)>& hasher, const auto& fallibility, const auto& layout) -> rusty::Result<rusty::Unit, TryReserveError> {
...
self_.growth_left = ::bucket_mask_to_capacity(std::move(self_.bucket_mask)) - rusty::detail::deref_if_pointer_like(self_.items);
    ...
    for (auto&& i : rusty::for_in(rusty::range(0, guard.num_buckets()))) {
        ...
        while (true) {
            ...
            assert((prev_ctrl == Tag :: DELETED));
            // @unsafe
            {
                ptr::swap_nonoverlapping(std::move(i_p), std::move(new_i_p), std::move(size_of));
            }
        }
    }
...
void RawTableInner::erase(size_t index) {
    ...
    auto [empty_before, empty_after] = rusty::detail::deref_if_pointer_like(std::make_tuple(Group::load(this->ctrl(std::move(index_before))).match_empty(), Group::load(this->ctrl(std::move(index))).match_empty()));
    auto ctrl = [&]() {
if ((rusty::leading_zeros(empty_before) + rusty::trailing_zeros(empty_after)) >= rusty::clone(Group::WIDTH)) {
...
RawIterHashIndices RawIterHashIndices::new_(const RawTableInner& table, uint64_t hash) {
    ...
    auto bitmask = rusty::iter(group.match_tag(std::move(tag_hash)));
...
rusty::Option<size_t> RawIterHashIndices::next() {
    ...
    // @unsafe
    {
        while (true) {
            ...
            assert((rusty::detail::deref_if_pointer_like(index) < ((rusty::detail::deref_if_pointer_like(this->bucket_mask) + 1) + rusty::clone(Group::WIDTH))));
            const auto group_ctrl = rusty::ptr::add(rusty::as_ptr(this->ctrl), std::move(index))->cast();
            this->group = Group::load(std::move(group_ctrl));
            this->bitmask = rusty::iter(this->group.match_tag(this->tag_hash));
        }
    }
}
