...
return rusty::Option<std::tuple<const K&, const V&>>(std::tuple<const K&, const V&>{r._0, r._1}); }(); } if (_m.is_none()) { return rusty::Option<std::tuple<const K&, const V&>>{rusty::None}; } return [&]() -> rusty::Option<std::tuple<const K&, const V&>> { rusty::intrinsics::unreachable(); }(); }();
    ...
    template<typename B, typename F, typename K, typename V>
    B fold(B init, F f) {
...
return f(std::move(acc), std::make_tuple(std::move(k), std::move(v))); }(); });
    ...
    template<typename K>
    static Iter<T> default_() {
        return Iter<T>{.iter = Iter::default_()};
    }
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    template<typename K>
...
struct IterMut {
    ...
    template<typename K, typename V>
    Iter<K, V> iter() const {
        return Iter<K, V>{.inner = rusty::clone(this->inner), .marker = rusty::PhantomData<const K&>{}};
    }
    template<typename K, typename V>
    static IterMut<T> default_() {
        return IterMut<T>{.inner = rusty::default_value<raw::RawIter<T>>(), .marker = rusty::PhantomData<T&>{}};
...
struct UnsafeIter {
    ...
    static UnsafeIter<T> default_() {
        return UnsafeIter<T>{.inner = rusty::default_value<raw::RawIter<T>>(), .marker = rusty::PhantomData<const rusty::Unit&>{}};
    }
    rusty::Option<Item> next() {
...
struct IntoIter {
    ...
    template<typename K, typename V>
    Iter<K, V> iter() const {
        return Iter<K, V>{.inner = rusty::iter(this->inner), .marker = rusty::PhantomData<const K&>{}};
    }
    template<typename K, typename V>
    static IntoIter<T, A> default_() {
        return IntoIter<T, A>{.inner = rusty::default_value<raw::RawIntoIter<T, A>>()};
    ...
    template<typename K>
    static IntoIter<T, A> default_() {
        return IntoIter<T, A>{.iter_field = IntoIter::default_()};
    }
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    template<typename K>
    rusty::Option<K> next() {
        return [&]() -> rusty::Option<K> { auto&& _m = this->iter.next(); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<K>(std::move(k)); } if (_m.is_none()) { return rusty::Option<K>{rusty::None}; } return [&]() -> rusty::Option<K> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename K>
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
        return this->iter.size_hint();
...
return f(std::move(acc), std::move(k));
    ...
    template<typename K>
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        const auto entries_iter = rusty::map(rusty::iter(this->iter), [&](auto&& _destruct_param0) {
...
struct HashTable {
    ...
    ExtractIf<T, F, A> extract_if(F f) {
        return ExtractIf<T, F, A>{.f = std::move(f), .inner = raw::RawExtractIf<T, A>{.iter = rusty::iter(this->raw), .table = &this->raw}};
    }
    template<size_t N>
        requires (rusty::alloc::Allocator<A>)
    ...
    static HashTable<T, A> default_() {
        return HashTable<T, A>{.raw = rusty::default_value<raw::RawTable<T, A>>()};
    }
...
return entry.insert(std::move(value)); }(); } return [&]() -> V& { rusty::intrinsics::unreachable(); }(); }();
    ...
    template<typename F, typename K, typename V, typename S>
    Entry<T, A> and_modify(F f) {
        return [&]() -> Entry<T, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> Entry<T, A> { f(entry.get_mut());
return Entry<T, A>{Entry_Occupied<T, A>{entry}}; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return Entry<T, A>{Entry_Vacant<T, A>{entry}}; } return [&]() -> Entry<T, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename F, typename K, typename V, typename S>
    Entry<T, A> and_replace_entry_with(F f) {
        return [&]() -> Entry<T, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.replace_entry_with(std::move(f)); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { return this->clone(); } return [&]() -> Entry<T, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename K, typename V, typename S>
...
struct OccupiedEntry {
    ...
    std::tuple<K, V> remove_entry() {
        // @unsafe
        {
            return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(this->table.table.remove(std::move(this->elem)));
        }
    }
    template<typename K, typename V, typename S>
    const V& get() const {
...
if (auto&& _iflet_scrutinee = f(key, std::move(value)); _iflet_scrutinee.is_some()) {
    ...
    return rusty::None;
            ...
            if (spare_key.is_some()) {
                ...
                return Entry<K, V, S, A>{Entry_Vacant<K, V, S, A>{VacantEntry<K, V>{.table = this->table, .hash = std::move(this->hash), .key_field = std::move(key)}}};
            } else {
...
struct VacantEntry {
    ...
    OccupiedEntry<K, V, S, A> insert_entry(V value) {
        ...
        return OccupiedEntry<K, V, S, A>{.table = this->table, .hash = std::move(this->hash), .elem = std::move(elem)};
    }
    template<typename K, typename V, typename S>
    rusty::HashMap<K, V, S, A>& into_map() {
        return this->table;
