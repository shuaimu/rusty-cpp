...
struct Iter {
    ...
    template<typename V>
    Iter<K> clone() const {
        return Iter<K>{.inner = rusty::clone(this->inner), .marker = rusty::PhantomData<rusty::Unit>{}};
    }
    template<typename V>
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return f.debug_list().entries(rusty::clone((*this))).finish();
    ...
    template<typename V>
    static Iter<K> default_() {
        return Iter<K>{.inner = Iter::default_(), .marker = rusty::PhantomData<rusty::Unit>{}};
    }
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    template<typename V>
...
struct IntoIter {
    ...
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    map::IntoIter<K, rusty::Unit, A> iter_field;

    static IntoIter<K, A> default_() {
        return IntoIter<K, A>{.iter_field = rusty::default_value<map::IntoIter<K, rusty::Unit, A>>()};
    }
    rusty::Option<K> next() {
        return [&]() -> rusty::Option<K> { auto&& _m = this->iter_field.next(); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<K>(std::move(k)); } if (_m.is_none()) { return rusty::Option<K>{rusty::None}; } return [&]() -> rusty::Option<K> { rusty::intrinsics::unreachable(); }(); }();
    }
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
    ...
    template<typename B, typename F>
    B fold(B init, F f) {
        return rusty::fold(this->iter_field, std::move(init), [&](auto&& acc, auto&& _destruct_param1) {
...
return f(std::move(acc), std::move(k));
    ...
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        const auto entries_iter = rusty::map(rusty::iter(this->iter_field), [&](auto&& _destruct_param0) {
...
return std::move(k);
    ...
    template<typename V>
    Iter<K, V> iter() const {
        return Iter<K, V>{.inner = rusty::iter(this->inner), .marker = rusty::PhantomData<rusty::Unit>{}};
    }
    template<typename V>
    static IntoIter<K, A> default_() {
        return IntoIter<K, A>{.inner = IntoIter::default_()};
    }
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    template<typename V>
...
struct Drain {
    ...
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    // Rust-only associated type alias with unbound generic skipped in constrained mode: Item
    map::Drain<K, rusty::Unit, A> iter_field;
...
auto [k, []] = rusty::detail::deref_if_pointer_like(_closure_ref_param0);
...
export template<typename T, typename S, typename A = rusty::alloc::Global>
    requires (rusty::alloc::Allocator<A>)
struct OccupiedEntry {
    map::OccupiedEntry<T, rusty::Unit, S, A> inner;
    ...
    std::tuple<K, V> remove_entry() {
        // @unsafe
        {
            return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(this->table.table.remove(rusty::detail::deref_if_pointer_like(this->elem)));
        }
    }
    template<typename K, typename V>
    const V& get() const {
...
if (auto&& _iflet_scrutinee = f(key, std::move(value)); _iflet_scrutinee.is_some()) {
    ...
    return rusty::None;
            ...
            if (spare_key.is_some()) {
                ...
                return Entry<K, V, S, A>{Entry_Vacant<K, V, S, A>{VacantEntry<K, V, S>{.hash = std::move(this->hash), .key_field = std::move(key), .table = std::move(this->table)}}};
            } else {
    ...
    Entry<T, A> replace_entry_with(F f) {
        // @unsafe
        {
            return [&]() -> Entry<T, A> { auto&& _m = this->table.raw.replace_bucket_with(rusty::clone(this->bucket), std::move(f)); if (_m.is_none()) { return Entry<T, A>{Entry_Occupied<T, A>{std::move((*this))}}; } if (_m.is_some()) { auto&& _mv1 = _m.unwrap(); auto&& tag = rusty::detail::deref_if_pointer(_mv1); return Entry<T, A>{Entry_Vacant<T, A>{VacantEntry<T, A, A>{.tag = std::move(tag), .index = this->bucket_index(), .table = std::move(this->table)}}}; } return [&]() -> Entry<T, A> { rusty::intrinsics::unreachable(); }(); }();
        }
    }
};
...
export template<typename T, typename S, typename A = rusty::alloc::Global>
    requires (rusty::alloc::Allocator<A>)
struct VacantEntry {
    map::VacantEntry<T, rusty::Unit, S, A> inner;
    ...
    V& insert(V value) {
        ...
        const auto entry = table.insert_entry(std::move(this->hash), std::make_tuple(std::move(this->key), std::move(value)), make_hasher<auto, V, S>(&this->table.hash_builder));
    ...
    OccupiedEntry<K, V, S, A> insert_entry(V value) {
        auto elem = this->table.table.insert(std::move(this->hash), std::make_tuple(std::move(this->key), std::move(value)), make_hasher<auto, V, S>(&this->table.hash_builder));
        return OccupiedEntry<K, V, S, A>{.hash = std::move(this->hash), .elem = std::move(elem), .table = std::move(this->table)};
    }
    template<typename K, typename V>
    rusty::HashMap<K, V, S, A>& into_map() {
        return std::move(this->table);
    ...
    OccupiedEntry<T, A> insert(T value) {
        ...
        return OccupiedEntry<T, A>{.bucket = std::move(bucket), .table = std::move(this->table)};
    }
    HashTable<T, A>& into_table() {
        return std::move(this->table);
    }
...
return entry; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry(std::move(value)); } return [&]() -> OccupiedEntry<K, V, S, A> { rusty::intrinsics::unreachable(); }(); }();
    ...
    template<typename F, typename K>
    decltype(auto) or_insert_with_key(F default_) {
        using V = std::remove_cvref_t<std::invoke_result_t<F&, const K&>>;
...
return entry.insert(std::move(value)); }(); } return [&]() -> V& { rusty::intrinsics::unreachable(); }(); }();
    ...
    template<typename F, typename K, typename V>
    Entry<T, S, A> and_modify(F f) {
        return [&]() -> Entry<T, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> Entry<T, S, A> { f(entry.get_mut());
return Entry<T, S, A>{Entry_Occupied<T, S, A>{entry}}; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return Entry<T, S, A>{Entry_Vacant<T, S, A>{entry}}; } return [&]() -> Entry<T, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename F, typename K, typename V>
    Entry<T, S, A> and_replace_entry_with(F f) {
        return [&]() -> Entry<T, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.replace_entry_with(std::move(f)); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { return this->clone(); } return [&]() -> Entry<T, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    template<typename K, typename V>
    rusty::HashMap<K, V, S, A>& into_map() {
    ...
    template<typename K, typename V>
    V& or_default() {
        return [&]() -> V& { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry.into_mut(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert(rusty::default_value<T>()); } return [&]() -> V& { rusty::intrinsics::unreachable(); }(); }();
    }
    OccupiedEntry<T, A> insert(T value) {
...
return entry; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert(std::move(value)); } return [&]() -> OccupiedEntry<T, A> { rusty::intrinsics::unreachable(); }(); }();
    ...
    Entry<T, S, A> and_modify(const auto& f) {
        return [&]() -> Entry<T, S, A> { auto&& _m = (*this); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> Entry<T, S, A> { f(entry.get_mut());
return Entry<T, S, A>{Entry_Occupied<T, S, A>{entry}}; }(); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return Entry<T, S, A>{Entry_Vacant<T, S, A>{entry}}; } return [&]() -> Entry<T, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    HashTable<T, A>& into_table() {
...
export template<typename T, typename S = DefaultHashBuilder, typename A = rusty::alloc::Global>
    requires (rusty::alloc::Allocator<A>)
struct HashSet {
    using Item = T;
    using IntoIter = ::IntoIter<T, A>;
    rusty::HashMap<T, rusty::Unit, S, A> map;
    ...
    static HashSet<T, S, A> new_() {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>());
    }
    static HashSet<T, S, A> with_capacity(size_t capacity) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::with_capacity(std::move(capacity)));
    }
    static HashSet<T, S, A> new_in(A alloc) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::new_in(std::move(alloc)));
    }
    static HashSet<T, S, A> with_capacity_in(size_t capacity, A alloc) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::with_capacity_in(std::move(capacity), std::move(alloc)));
    }
    ...
    Drain<T, A> drain() {
        return Drain<T, A>{.iter_field = this->map.drain()};
    }
    template<typename F>
    void retain(F f) {
    ...
    ExtractIf<T, F, A> extract_if(F f) {
        return ExtractIf<T, F, A>{.f = std::move(f), .inner = raw::RawExtractIf<std::tuple<T, rusty::Unit>, A>{.iter = rusty::iter(this->map.table), .table = &this->map.table}};
    }
    void clear() {
        this->map.clear();
    }
    static HashSet<T, S, A> with_hasher(S hasher) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::with_hasher(std::move(hasher)));
    }
    static HashSet<T, S, A> with_capacity_and_hasher(size_t capacity, S hasher) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::with_capacity_and_hasher(std::move(capacity), std::move(hasher)));
    }
    const A& allocator() const {
        return this->map.allocator();
    }
    static HashSet<T, S, A> with_hasher_in(S hasher, A alloc) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::with_hasher_in(std::move(hasher), std::move(alloc)));
    }
    static HashSet<T, S, A> with_capacity_and_hasher_in(size_t capacity, S hasher, A alloc) {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::with_capacity_and_hasher_in(std::move(capacity), std::move(hasher), std::move(alloc)));
    }
    ...
    Difference<T, S, A> difference(const HashSet<T, S, A>& other) const {
        return Difference<T, S, A>(rusty::iter((*this)), other);
    }
    ...
    Intersection<T, S, A> intersection(const HashSet<T, S, A>& other) const {
        auto [smaller, larger] = rusty::detail::deref_if_pointer_like((rusty::len((*this)) <= rusty::len(other) ? std::make_tuple((*this), other) : std::make_tuple(other, (*this))));
    ...
    rusty::Option<const T&> get(const Q& value) const {
        return [&]() -> rusty::Option<const T&> { auto&& _m = this->map.get_key_value(value); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<const T&>(k); } if (_m.is_none()) { return rusty::Option<const T&>{rusty::None}; } return [&]() -> rusty::Option<const T&> { rusty::intrinsics::unreachable(); }(); }();
    }
    const T& get_or_insert(T value) {
        return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })([&]() { auto&& _m = this->map.entry(std::move(value)); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry(std::make_tuple()); } rusty::intrinsics::unreachable(); }().into_entry());
    }
    template<typename Q, typename F>
        requires (rusty::alloc::Allocator<A>)
    const T& get_or_insert_with(const Q& value, F f) {
        return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })([&]() { auto&& _m = this->map.entry_ref(value); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return entry; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return entry.insert_entry_with_key(f(value), std::make_tuple()); } rusty::intrinsics::unreachable(); }().into_entry());
    }
    Entry<T, S, A> entry(T value) {
        return [&]() -> Entry<T, S, A> { auto&& _m = this->map.entry(std::move(value)); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto&& entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return Entry<T, S, A>{Entry_Occupied<T, S, A>{OccupiedEntry<T, S, A>{.inner = std::move(entry)}}}; } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return Entry<T, S, A>{Entry_Vacant<T, S, A>{VacantEntry<T, S, A>{.inner = std::move(entry)}}}; } return [&]() -> Entry<T, S, A> { rusty::intrinsics::unreachable(); }(); }();
    }
    ...
    bool is_subset(const HashSet<T, S, A>& other) const {
        return (rusty::len((*this)) <= rusty::len(other)) && rusty::iter((*this)).all([&](auto&& v) { return rusty::contains(other, std::move(v)); });
    }
    ...
    const T& insert_unique_unchecked(T value) {
        // @unsafe
        {
            return ([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(this->map.insert_unique_unchecked(std::move(value), std::make_tuple()));
        }
    }
    rusty::Option<T> replace(T value) {
        auto value_shadow1 = UnsafeCell::new_(std::move(value));
        return [&]() -> rusty::Option<T> { auto&& _m = this->map.entry_ref(rusty::detail::deref_if_pointer_like(value_shadow1.get())); if (rusty::detail::deref_if_pointer(_m).index() == 1) { auto entry = rusty::detail::deref_if_pointer(std::get<1>(rusty::detail::deref_if_pointer(_m))._0); return rusty::Option<T>(entry.replace_key_unchecked(value_shadow1.into_inner())); } if (rusty::detail::deref_if_pointer(_m).index() == 0) { auto&& entry = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_m))._0); return [&]() -> rusty::Option<T> { // @unsafe
{
    entry.insert_with_key_unchecked(value_shadow1.into_inner(), std::make_tuple());
}
return rusty::Option<T>{rusty::None}; }(); } return [&]() -> rusty::Option<T> { rusty::intrinsics::unreachable(); }(); }();
    ...
    rusty::Option<T> take(const Q& value) {
        return [&]() -> rusty::Option<T> { auto&& _m = this->map.remove_entry(value); if (_m.is_some()) { auto&& _mv0 = _m.unwrap(); auto&& k = rusty::detail::deref_if_pointer(std::get<0>(rusty::detail::deref_if_pointer(_mv0))); return rusty::Option<T>(std::move(k)); } if (_m.is_none()) { return rusty::Option<T>{rusty::None}; } return [&]() -> rusty::Option<T> { rusty::intrinsics::unreachable(); }(); }();
    }
    ...
    bool operator==(const HashSet<T, S, A>& other) const {
        ...
        return rusty::iter((*this)).all([&](auto&& key) { return rusty::contains(other, std::move(key)); });
    }
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return f.debug_set().entries(rusty::iter((*this))).finish();
    }
    static HashSet<T, S, A> from(rusty::HashMap<T, rusty::Unit, S, A> map) {
        return HashSet<T, S, A>(std::move(map));
    }
    template<typename I>
        requires (rusty::alloc::Allocator<A>)
    static HashSet<T, S, A> from_iter(I iter) {
        auto set = HashSet<T, S, A>::with_hasher_in(rusty::default_value<S>(), rusty::default_value<A>());
        set.extend(std::move(iter));
        return std::move(set);
    }
    template<size_t N>
        requires (rusty::alloc::Allocator<A>)
    static HashSet<T, S, A> from(std::array<T, rusty::sanitize_array_capacity<N>()> arr) {
        return HashSet<T, S, A>::from_iter(rusty::iter(arr));
    }
    template<typename I>
        requires (rusty::alloc::Allocator<A>)
    void extend(I iter) {
    ...
    void extend_one(T k) {
        this->map.insert(std::move(k), std::make_tuple());
    }
    void extend_reserve(size_t additional) {
        (&this->map)->extend_reserve(std::move(additional));
    }
    void extend_one(const T& k) {
        this->map.insert(k, std::make_tuple());
    }
    static HashSet<T, S, A> default_() {
        return HashSet<T, S, A>(rusty::HashMap<T, rusty::Unit, S, A>::default_());
    }
    HashSet<T, S, A> operator|(const HashSet<T, S, A>& rhs) const {
        return HashSet<T, S, A>::from_iter(this->union_(rhs).cloned());
    }
    HashSet<T, S, A> operator&(const HashSet<T, S, A>& rhs) const {
        return HashSet<T, S, A>::from_iter(this->intersection(rhs).cloned());
    }
    HashSet<T, S, A> operator^(const HashSet<T, S, A>& rhs) const {
        return HashSet<T, S, A>::from_iter(this->symmetric_difference(rhs).cloned());
    }
    HashSet<T, S, A> operator-(const HashSet<T, S, A>& rhs) const {
        return HashSet<T, S, A>::from_iter(this->difference(rhs).cloned());
    }
    ...
    void operator^=(const HashSet<T, S, A>& rhs) {
        for (auto&& item : rusty::for_in(rusty::iter(rhs))) {
            {
                auto&& _m = this->map.entry_ref(std::move(item));
                std::visit(overloaded {
                    [&](const std::variant_alternative_t<0, rusty::detail::variant_underlying_type_t<decltype(rusty::detail::deref_if_pointer(_m))>>& _v) {
                        auto&& entry = rusty::detail::deref_if_pointer(_v._0);
                        entry.remove();
                    },
    ...
    void operator-=(const HashSet<T, S, A>& rhs) {
        if (rusty::len(rhs) < rusty::len((*this))) {
            for (auto&& item : rusty::for_in(rusty::iter(rhs))) {
    ...
    IntoIter into_iter() {
        return IntoIter{.iter_field = this->map.into_iter()};
    }
};
...
struct Intersection {
    using Item = const T&;
    Iter<T> iter;
    const rusty::HashSet<T, S, A>& other;
...
struct Difference {
    ...
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
        ...
        return std::make_tuple(rusty::saturating_sub(lower, rusty::detail::deref_if_pointer(rusty::len(this->other))), std::move(upper));
    }
    template<typename B, typename F>
        requires (rusty::alloc::Allocator<A>)
    B fold(B init, F f) {
...
struct SymmetricDifference {
    using Item = const T&;
    std::iter::Chain<Difference<T, S, A>, Difference<T, S, A>> iter;
...
struct Union {
    using Item = const T&;
    std::iter::Chain<Iter<T>, Difference<T, S, A>> iter;
...
void assert_covariance() {
    const rusty::SafeFn<rusty::HashSet<std::string_view>(rusty::HashSet<std::string_view>)> set = +[](rusty::HashSet<std::string_view> v) -> rusty::HashSet<std::string_view> {
        return std::move(v);
    };
