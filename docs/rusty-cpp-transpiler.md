...
17. New first deterministic Stage D head in `smallvec` now starts at `runner.cpp:1474` with unspecialized owner-template surface `NonNull::new_(...)` (missing template specialization), followed by bound-associated variant/type fallout (`Included`/`Excluded` unresolved in `drain` bound lowering and downstream unresolved `T` at `SmallVec<T>` call surfaces).
18. Next active item is `Leaf 5.1.11` to collapse this post-5.1.10 owner-template/bound-type specialization family; guardrail check against §11 remains satisfied (shared AST/runtime-surface-aware fixes only, no crate-specific ad-hoc scripts or generated-output text patching).
