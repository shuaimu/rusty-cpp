...
### 10.100 Leaf 4.15.4.3.3.3.3.3.7.2 — Full Matrix Re-Run and Next Deterministic Frontier After 7.1

Scope completed in this pass:

- Executed the leaf exactly as defined: re-ran the full seven-crate matrix after 7.1 and
  recorded the next deterministic blocker frontier.
- This leaf remained procedural/documentation scope (no new transpiler/runtime implementation),
  so it stayed well under the 1000-LOC guideline.

Execution and findings:

- Ran `tests/transpile_tests/run_parity_matrix.sh` (full matrix).
- Stable result order remains:
  - pass: `either`, `tap`, `cfg-if`, `take_mut`
  - first fail: `arrayvec` (Stage D C++ build)
  - matrix summary at failure: `total=5`, `pass=4`, `fail=1`
- Deterministic first hard-error frontier in `arrayvec` Stage D now starts at downstream
  iterator-state/runtime-shape fallout:
  - `IntoIter::next` self-field shadow emission (`const auto index = index;`),
  - dependent next-iterator adaptation failures in shared runtime path
    (`option_take_value(std::nullopt_t)`, invalid `current_` use, missing `begin/end` on
    `fold(map(...))` instantiations),
  - then later dependent span/data/option-shape families
    (`&std::array{...}` to `std::span<const T>`, `slice_full`/`std::data` on `ArrayVec`,
    `rusty::Option<T>` vs `std::optional<U>` comparison fallout).

Task decomposition decision:

- Leaf 7.2 itself did not require decomposition.
- The newly exposed implementation frontier was split in TODO as
  `4.15.4.3.3.3.3.3.8` with `8.1/8.2` so each executable leaf remains below the LOC target.
...
- §11.91/§11.92: no declaration-order or output-text surgery to force matrix success.
- §11.82: no crate-specific runtime patching in this procedural leaf.
