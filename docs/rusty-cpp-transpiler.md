...
**Invariants (carry forward from §13.8/§13.10).** (1) *Engine-first, heuristic-fallback, never-crash* — a `None` or unresolved variable falls back to today's path unchanged. (2) *Flagged rollout* — `RUSTY_CPP_INFER_ENGINE`, default-off, flipped on only after a clean matrix (mirrors the UFCS flag precedent, Ch. 14 / §UFCS). (3) *Matrix-gated, retire-not-add* — every step ends with the parity matrix no worse than today **and the `augment_*` heuristic count strictly lower**. If a step cannot retire its heuristic without regression, the engine rule is incomplete; finish the rule rather than keep both.
...
**C. Widen the collector — one rule per shape, each retiring a heuristic.** Add the rule + a unit test → matrix → **delete the heuristic** → matrix.

| # | Constraint rule (in `ConstraintCollector`) | Heuristic it retires | Unblocks |
|---|---|---|---|
| C1 | Struct/variant construction: `Struct{f: e}` ⇒ `typeof(e)=field_ty`; `Wrapper::from/new(x)` ⇒ `x=field_ty`. (`StructFieldInit` origin already exists at type_solver.rs:118, never pushed.) | `augment_vec_local_element_hints_from_newtype_consumer` | bytebuf **+** bytearray array-field **+** Cow, from one rule |
| C2 | Call signatures: resolve callee → unify args↔params, result↔return. Generalize the `ItemResolver` callback into a `SignatureResolver`. | `augment_local_generic_placeholder_hints_from_function_calls`, much of `augment_uninitialized_*` | the long tail |
...
After C1–C5, bytebuf/bytearray/Cow all fall out of the *same* constraint set (field-init + assignment + for-iter + `?`) rather than three bespoke passes — the point of the chapter.

**D. Telemetry + acceptance.** Implement the deferred §13.10 dump as `RUSTY_CPP_PRINT_INFERENCE` (collected constraints, solve iterations, final substitution, and every emit-site fallback). Frequent fallbacks name the next rule to write. Acceptance: §13.10 met, matrix ≥ today (14 pass / 1 known-fail), `augment_*` count strictly decreasing per step, and the serde_bytes test targets (`test_derive`/`test_serde`) genuinely compile (closes #35/#36).

**E. Risks.** A2 identity is the crux — prove the positional index consistent between collect and emit (with a unit test that resolves a known local) before touching any emit path. Big-bang risk is contained by the flag + per-step heuristic retirement. The `stmts.len() <= 128` augmentation guard (emit_expr.rs:532) has an analogue here: per-function solve is O(constraints) and should be cheaper than the N forward-scans it replaces, but keep a ceiling and measure. The work does not vanish — it relocates from "new pass" to "new rule" — but rules **compose in one solver** instead of multiplying as blind passes, so the net heuristic count falls.
