...
26. New first deterministic Stage D head in `smallvec` now starts at `runner.cpp:1345` (`SetLenOnDrop` destructor pointer/reference lowering mismatch: invalid unary `*` against `size_t&`-lowered storage), followed by adjacent `ConstNonNull::new_` cast/call-shape fallout (`runner.cpp:1354`) and downstream callable-resolution fallout (`runner.cpp:2288` `move(for_each)` unresolved overload shape).
27. Next active item is `Leaf 5.1.16` to collapse this post-5.1.15 len-guard/callable-shape family; guardrail check against §11 remains satisfied (shared AST/runtime-surface-aware fixes only, no crate-specific ad-hoc scripts or generated-output text patching).
