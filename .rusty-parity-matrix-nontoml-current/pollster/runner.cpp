...
struct Signal;
template<typename F>
typename F::Output block_on(F fut);
...
// Rust-only libtest metadata const skipped: mpsc (marker: mpsc, should_panic: no)

void basic() {
    const auto make_fut = [&]() { return rusty::future::ready(42); };
    {
        auto&& _m0_tmp = block_on(make_fut());
        auto _m0 = &_m0_tmp;
    ...
    const auto then = Instant::now();
    block_on(rusty::future::Delay::new_(Duration::from_millis(250)));
    if (!(Instant::now().duration_since(std::move(then)) > Duration::from_millis(250))) {
        rusty::panicking::panic("assertion failed: Instant::now().duration_since(then) > Duration::from_millis(250)");
    }
}
...
// Rust-only libtest marker without emitted function: mpsc
