...
namespace lib {
    ...
    export using ::rusty::Rc;
    export template<typename... Ts> using RcWeak = rusty::Weak<Ts...>;

    export using ::rusty::Arc;
    export template<typename... Ts> using ArcWeak = rusty::sync::Weak<Ts...>;
...
return rusty::Result<typename S::Ok, typename S::Error>::Err(err); } std::move(_match_value).value(); });
            ...
            rusty::Result<typename S::Ok, typename S::Error> serialize(const rusty::Weak<T>& self_, S serializer) {
                using Self = std::remove_reference_t<decltype(self_)>;
                return ([&](auto&& __self) -> decltype(auto) { if constexpr (requires { ::ser::impls::rusty_ext::serialize(std::forward<decltype(__self)>(__self), std::move(serializer)); }) { return ::ser::impls::rusty_ext::serialize(std::forward<decltype(__self)>(__self), std::move(serializer)); } else { return ::ser::impls::rusty_ext::serialize(rusty::detail::deref_if_pointer_like(std::forward<decltype(__self)>(__self)), std::move(serializer)); } })(self_.upgrade());
            }
