...
namespace scoped { struct Scope; }
namespace scoped { template<typename T, typename F> struct Hole; }
...
namespace scoped {
    ...
    template<typename F>
    auto scope(F f);
    void scope_based_take();
    void panic_on_recovered_panic();
}
template<typename T, typename F>
void take(T& mut_ref, F closure);
void it_works();
template<typename T, typename F, typename R>
void take_or_recover(T& mut_ref, R recover, F closure);
void it_works_recover();
void it_works_recover_panic();
...
namespace scoped {
    ...
    // Rust-only unresolved import: using std;
    ...
    struct Scope {
        rusty::Cell<size_t> active_holes;
        rusty::PhantomData<rusty::Cell<std::tuple<>&>> marker;

        template<typename T, typename F>
        std::tuple<T, Hole<T, F>> take_or_recover(T& mut_ref, F recovery) const;
        template<typename T>
        std::tuple<T, Hole<T, rusty::SafeFn<T()>>> take(T& mut_ref) const;
    };
    ...
    template<typename T, typename F>
    struct Hole {
        const rusty::Cell<size_t>& active_holes;
        std::add_pointer_t<T> hole;
        rusty::PhantomData<T&> phantom;
        rusty::Option<F> recovery;
        Hole(const rusty::Cell<size_t>& active_holes_init, std::add_pointer_t<T> hole_init, rusty::PhantomData<T&> phantom_init, rusty::Option<F> recovery_init) : active_holes(active_holes_init), hole(std::move(hole_init)), phantom(std::move(phantom_init)), recovery(std::move(recovery_init)) {}
        Hole(const Hole&) = default;
        Hole(Hole&& other) noexcept : active_holes(other.active_holes), hole(std::move(other.hole)), phantom(std::move(other.phantom)), recovery(std::move(other.recovery)) {
        ...
        void fill(T t) {
            ...
            // @unsafe
            {
                rusty::ptr::write(std::move(this->hole), std::move(t));
            }
            const auto num_holes = this->active_holes.get();
            this->active_holes.set(num_holes - 1);
            rusty::mem::forget(std::move((*this)));
        }
        ~Hole() noexcept(false) {
            if (rusty::mem::consume_forgotten_address(this)) { return; }
            namespace ptr = rusty::ptr;
            auto t = (this->recovery.take().expect("No recovery function in Hole!"))();
            // @unsafe
            {
                rusty::ptr::write(this->hole, std::move(t));
            }
            const auto num_holes = this->active_holes.get();
            this->active_holes.set(num_holes - 1);
        }
    ...
    // Rust-only libtest metadata const skipped: scope_based_take (marker: scoped::scope_based_take, should_panic: no)


    // Rust-only libtest metadata const skipped: panic_on_recovered_panic (marker: scoped::panic_on_recovered_panic, should_panic: no)

}

// Rust-only libtest metadata const skipped: it_works (marker: it_works, should_panic: no)

// Rust-only libtest metadata const skipped: it_works_recover (marker: it_works_recover, should_panic: no)

// Rust-only libtest metadata const skipped: it_works_recover_panic (marker: it_works_recover_panic, should_panic: no)
...
void take(T& mut_ref, F closure) {
    ...
    // @unsafe
    {
        const auto old_t = rusty::ptr::read(&mut_ref);
        auto new_t = rusty::panic::catch_unwind(rusty::panic::AssertUnwindSafe([&]() { return closure(std::move(old_t)); })).unwrap_or_else([&](auto _closure_wild0) { std::abort(); });
        rusty::ptr::write(&mut_ref, std::move(new_t));
    }
}
...
void it_works() {
    ...
    const auto drop = [&](Foo& self_) {
        switch (self_) {
        case Foo::A:
        {
            rusty::io::_print(std::string("Foo::A dropped\n"));
            break;
        }
        case Foo::B:
        {
            rusty::io::_print(std::string("Foo::B dropped\n"));
            break;
        }
        }
    };
    ...
    // Rust-only nested impl block skipped in local scope
    // Rust-only nested impl block skipped in local scope
    auto foo = rusty::clone(rusty::clone(Foo::A));
    ::take(foo, [&](auto&& f) {
rusty::mem::drop(std::move(f));
return rusty::clone(Foo::B);
});
    {
        auto _m0 = &foo;
        auto&& _m1_tmp = rusty::clone(Foo::B);
        auto _m1 = &_m1_tmp;
...
void take_or_recover(T& mut_ref, R recover, F closure) {
    ...
    {
        const auto old_t = rusty::ptr::read(&mut_ref);
        auto new_t = rusty::panic::catch_unwind(rusty::panic::AssertUnwindSafe([&]() { return closure(std::move(old_t)); }));
        {
            auto&& _m = new_t;
            bool _m_matched = false;
            if (!_m_matched) {
                if (_m.is_err()) {
                    auto&& _mv0 = _m.unwrap_err();
                    auto&& err = rusty::detail::deref_if_pointer(_mv0);
                    auto r = rusty::panic::catch_unwind(rusty::panic::AssertUnwindSafe([&]() { return recover(); })).unwrap_or_else([&](auto _closure_wild0) { std::abort(); });
                    rusty::ptr::write(&mut_ref, std::move(r));
                    rusty::panic::resume_unwind(err);
                    _m_matched = true;
                }
            }
            if (!_m_matched) {
                if (_m.is_ok()) {
                    auto&& _mv1 = _m.unwrap();
                    auto&& new_t = rusty::detail::deref_if_pointer(_mv1);
                    rusty::ptr::write(&mut_ref, std::move(new_t));
                    _m_matched = true;
                }
            }
...
void it_works_recover() {
    ...
    // Rust-only nested impl block skipped in local scope
    auto foo = rusty::clone(rusty::clone(Foo::A));
    ::take_or_recover(foo, [&]() { return rusty::clone(Foo::A); }, [&](auto&& f) {
rusty::mem::drop(std::move(f));
return rusty::clone(Foo::B);
});
    {
...
void it_works_recover_panic() {
    ...
    const auto drop = [&](Foo& self_) {
        ...
        case Foo::C:
        {
            rusty::io::_print(std::string("Foo::C dropped\n"));
            break;
        }
        }
    };
    ...
    // Rust-only nested impl block skipped in local scope
    auto foo = rusty::clone(rusty::clone(Foo::A));
    const auto res = rusty::panic::catch_unwind(rusty::panic::AssertUnwindSafe([&]() {
::take_or_recover(foo, [&]() { return rusty::clone(Foo::C); }, [&](auto&& f) {
rusty::mem::drop(std::move(f));
{
    rusty::panic::begin_panic("panic");
}
return rusty::clone(Foo::B);
});
}));
    ...
    {
        auto _m0 = &foo;
        auto&& _m1_tmp = rusty::clone(Foo::C);
        auto _m1 = &_m1_tmp;
        auto _m_tuple = std::make_tuple(_m0, _m1);
        bool _m_matched = false;
        if (!_m_matched) {
...
namespace scoped {
    ...
    template<typename F>
    auto scope(F f) {
        const auto this_ = Scope{.active_holes = rusty::Cell<size_t>::new_(static_cast<size_t>(0)), .marker = rusty::PhantomData<rusty::Cell<std::tuple<>&>>{}};
        const auto result = rusty::panic::catch_unwind(rusty::panic::AssertUnwindSafe([&]() {
return f(this_);
}));
        if (this_.active_holes.get() != 0) {
            std::abort();
        }
        return [&]() { auto&& _m = result; if (_m.is_ok()) { return _m.unwrap(); } if (_m.is_err()) { auto&& _mv1 = _m.unwrap_err(); auto&& p = rusty::detail::deref_if_pointer(_mv1); return rusty::panic::resume_unwind(std::move(p)); } rusty::intrinsics::unreachable(); }();
    }
    ...
    void scope_based_take() {
        ...
        // Rust-only nested impl block skipped in local scope
        // Rust-only nested impl block skipped in local scope
        auto bar = Bar{.a = Foo{}, .b = Foo{}};
        scope([&](auto&& scope) {
auto [a, a_hole] = rusty::detail::deref_if_pointer_like(scope.take(bar.a));
auto [b, b_hole] = rusty::detail::deref_if_pointer_like(scope.take(bar.b));
a_hole.fill(Foo{});
b_hole.fill(Foo{});
});
        {
            rusty::io::_print(std::format("{0}\n", rusty::to_debug_string(&bar)));
        }
    }
    ...
    void panic_on_recovered_panic() {
        ...
        const auto result = rusty::panic::catch_unwind(rusty::panic::AssertUnwindSafe([&]() {
scope([&](auto&& scope) {
auto [t, hole] = rusty::detail::deref_if_pointer_like(scope.take_or_recover(rusty::detail::deref_if_pointer_like(foo), [&]() { return Foo{}; }));
{
    rusty::panic::begin_panic("Oops!");
}
});
}));
        if (!result.is_err()) {
...
namespace scoped {
    template<typename T, typename F>
    std::tuple<T, Hole<T, F>> Scope::take_or_recover(T& mut_ref, F recovery) const {
        namespace ptr = rusty::ptr;
        std::optional<T> t;
        std::optional<Hole<T, F>> hole;
        const auto num_of_holes = this->active_holes.get();
        if (num_of_holes == std::numeric_limits<size_t>::max()) {
            {
                rusty::panic::begin_panic("Too many holes!");
            }
        }
        this->active_holes.set(num_of_holes + 1);
        // @unsafe
        {
            t.emplace(rusty::ptr::read(static_cast<std::add_pointer_t<T>>(&mut_ref)));
            hole.emplace(Hole<T, F>(this->active_holes, static_cast<std::add_pointer_t<T>>(&mut_ref), rusty::PhantomData<T&>{}, rusty::Option<F>(std::move(recovery))));
        }
        return std::make_tuple(std::move(t.value()), std::move(hole.value()));
    }
}

namespace scoped {
    template<typename T>
    std::tuple<T, Hole<T, rusty::SafeFn<T()>>> Scope::take(T& mut_ref) const {
        const rusty::SafeFn<T()> panic = +[]() -> T {
            {
                rusty::panic::begin_panic("Failed to recover a Hole!");
            }
        };
        return this->take_or_recover(mut_ref, std::move(panic));
    }
}


// Runnable wrappers for expanded Rust test bodies
// Rust-only libtest wrapper metadata: marker=scoped::scope_based_take should_panic=no
void rusty_test_scoped_scope_based_take() {
    scoped::scope_based_take();
}
// Rust-only libtest wrapper metadata: marker=scoped::panic_on_recovered_panic should_panic=no
void rusty_test_scoped_panic_on_recovered_panic() {
    scoped::panic_on_recovered_panic();
}
// Rust-only libtest wrapper metadata: marker=it_works should_panic=no
void rusty_test_it_works() {
    it_works();
}
// Rust-only libtest wrapper metadata: marker=it_works_recover should_panic=no
void rusty_test_it_works_recover() {
    it_works_recover();
}
// Rust-only libtest wrapper metadata: marker=it_works_recover_panic should_panic=no
void rusty_test_it_works_recover_panic() {
...
int main(int argc, char** argv) {
    if (argc == 3 && std::string(argv[1]) == "--rusty-single-test") {
        ...
        try {
            if (test_name == "rusty_test_it_works") { rusty_test_it_works(); return 0; }
            if (test_name == "rusty_test_it_works_recover") { rusty_test_it_works_recover(); return 0; }
            if (test_name == "rusty_test_it_works_recover_panic") { rusty_test_it_works_recover_panic(); return 0; }
            if (test_name == "rusty_test_scoped_panic_on_recovered_panic") { rusty_test_scoped_panic_on_recovered_panic(); return 0; }
            if (test_name == "rusty_test_scoped_scope_based_take") { rusty_test_scoped_scope_based_take(); return 0; }
    ...
    int pass = 0, fail = 0;
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_it_works(); std::cout << "  it_works PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  it_works FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  it_works FAILED (unknown exception)" << std::endl; fail++; }
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_it_works_recover(); std::cout << "  it_works_recover PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  it_works_recover FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  it_works_recover FAILED (unknown exception)" << std::endl; fail++; }
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_it_works_recover_panic(); std::cout << "  it_works_recover_panic PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  it_works_recover_panic FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  it_works_recover_panic FAILED (unknown exception)" << std::endl; fail++; }
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_scoped_panic_on_recovered_panic(); std::cout << "  scoped_panic_on_recovered_panic PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  scoped_panic_on_recovered_panic FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  scoped_panic_on_recovered_panic FAILED (unknown exception)" << std::endl; fail++; }
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_scoped_scope_based_take(); std::cout << "  scoped_scope_based_take PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  scoped_scope_based_take FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  scoped_scope_based_take FAILED (unknown exception)" << std::endl; fail++; }
    std::cout << std::endl;
