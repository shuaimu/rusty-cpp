...
WORK_DIR="${REPO_ROOT}/.rusty-cpp-module-interop"
DRY_RUN=0
...
Run compile-stage cpp:: interop coverage for the committed fixture crate.
...
report_failure() {
    local stage="$1"
    echo "cpp-module-interop compile check failed at stage: ${stage}" >&2
...
echo "═══════════════════════════════════════════════════════════════════════"
echo "cpp:: interop compile check"
...
if ! (
    cd "${WORK_DIR}" &&
    "${SUPPORTED_COMPILER}" "${ACTIVE_FLAGS[@]}" -I "${REPO_ROOT}/include" -x c++ -c "${CUSTOM_MODULE_PATH}" -o custom.math.o
) >>"${BUILD_LOG}" 2>&1; then
    report_failure "compile-custom-module"
...
if ! (
    cd "${WORK_DIR}" &&
    "${SUPPORTED_COMPILER}" "${ACTIVE_FLAGS[@]}" -I "${REPO_ROOT}/include" -x c++ -c "${TARGET_CPPM_PATH}" -o cpp_module_interop.o
) >>"${BUILD_LOG}" 2>&1; then
...
echo "PASS: cpp:: interop module compile check succeeded"
