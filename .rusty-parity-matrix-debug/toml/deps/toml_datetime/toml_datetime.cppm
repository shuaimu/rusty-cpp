...
namespace datetime {
    rusty::Result<Datetime, DatetimeParseError> Datetime::from_str(std::string_view date) {
        ...
        RUSTY_TRY_INTO(digits.is(TokenKind::Digits).map_err([&](auto&& err) { return err.expected(std::string_view("year or hour")); }), rusty::Result<Datetime, DatetimeParseError>);
...
return DatetimeParseError::new_().what(std::string_view("date")).expected(std::string_view("month"));
            ...
            RUSTY_TRY_INTO(month.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("date")).expected(std::string_view("month")); }), rusty::Result<Datetime, DatetimeParseError>);
            ...
            RUSTY_TRY_INTO(sep_shadow1.is(TokenKind::Dash).map_err([&](auto&& err) { return err.what(std::string_view("date")).expected(std::string_view("`-` (MM-DD)")); }), rusty::Result<Datetime, DatetimeParseError>);
            ...
            RUSTY_TRY_INTO(day.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("date")).expected(std::string_view("day")); }), rusty::Result<Datetime, DatetimeParseError>);
...
return DatetimeParseError::new_().what(std::string_view("time")).expected(std::string_view("hour"));
            ...
            RUSTY_TRY_INTO(hour.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("time")).expected(std::string_view("hour")); }), rusty::Result<Datetime, DatetimeParseError>);
            ...
            RUSTY_TRY_INTO(sep_shadow1.is(TokenKind::Colon).map_err([&](auto&& err) { return err.what(std::string_view("time")).expected(std::string_view("`:` (HH:MM)")); }), rusty::Result<Datetime, DatetimeParseError>);
            ...
            RUSTY_TRY_INTO(minute.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("time")).expected(std::string_view("minute")); }), rusty::Result<Datetime, DatetimeParseError>);
            ...
            {
                if (rusty::clone(lexer).next().map([&](auto&& t) { return t.kind; }) == rusty::Option<TokenKind>(TokenKind::Colon)) {
                    const auto sep = RUSTY_TRY_INTO(lexer.next().ok_or(DatetimeParseError::new_()), rusty::Result<Datetime, DatetimeParseError>);
                    RUSTY_TRY_INTO(sep.is(TokenKind::Colon), rusty::Result<Datetime, DatetimeParseError>);
                    ...
                    RUSTY_TRY_INTO(second_shadow1.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("time")).expected(std::string_view("second")); }), rusty::Result<Datetime, DatetimeParseError>);
            ...
            {
                if (second.is_some() && (rusty::clone(lexer).next().map([&](auto&& t) { return t.kind; }) == rusty::Option<TokenKind>(TokenKind::Dot))) {
                    const auto sep = RUSTY_TRY_INTO(lexer.next().ok_or(DatetimeParseError::new_()), rusty::Result<Datetime, DatetimeParseError>);
                    RUSTY_TRY_INTO(sep.is(TokenKind::Dot), rusty::Result<Datetime, DatetimeParseError>);
                    ...
                    RUSTY_TRY_INTO(nanosecond_shadow1.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("time")).expected(std::string_view("nanosecond")); }), rusty::Result<Datetime, DatetimeParseError>);
        ...
        if (result.date.is_some() && result.time.is_some()) {
            {
                ...
                if (!_m_matched) {
                    if (_m.is_some()) {
                        ...
                        auto&& token = _mv0;
                        if (token.kind == rusty::clone(TokenKind::Z)) {
                ...
                if (!_m_matched) {
                    if (_m.is_some()) {
                        ...
                        if (({ auto&& _m = token.kind; std::optional<bool> _match_value; bool _m_matched = false; if (!_m_matched && (_m == TokenKind::Plus || _m == TokenKind::Dash)) { _match_value.emplace(std::move(true)); _m_matched = true; } if (!_m_matched) { _match_value.emplace(std::move(false)); _m_matched = true; } if (!_m_matched) { rusty::intrinsics::unreachable(); } std::move(_match_value).value(); })) {
                            const auto sign = (token.kind == TokenKind::Plus ? static_cast<int32_t>(1) : -1);
                            ...
                            RUSTY_TRY_INTO(hours.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("offset")).expected(std::string_view("hour")); }), rusty::Result<Datetime, DatetimeParseError>);
                            ...
                            RUSTY_TRY_INTO(sep_shadow1.is(TokenKind::Colon).map_err([&](auto&& err) { return err.what(std::string_view("offset")).expected(std::string_view("`:` (HH:MM)")); }), rusty::Result<Datetime, DatetimeParseError>);
                            ...
                            RUSTY_TRY_INTO(minutes.is(TokenKind::Digits).map_err([&](auto&& err) { return err.what(std::string_view("offset")).expected(std::string_view("minute")); }), rusty::Result<Datetime, DatetimeParseError>);
...
namespace datetime {
    rusty::Option<Token> Lexer::unknown() {
        ...
        return rusty::Option<Token>(Token{.kind = TokenKind::Unknown, .raw = std::move(rusty::to_string_view(raw))});
    }
}

namespace datetime {
    rusty::Option<Token> Lexer::next() {
        auto [kind, end] = rusty::detail::deref_if_pointer_like(({ auto&& _m = RUSTY_TRY_OPT((!rusty::as_bytes(this->stream).empty() ? rusty::Option<const uint8_t&>(rusty::as_bytes(this->stream)[0]) : rusty::Option<const uint8_t&>(rusty::None))); std::optional<std::remove_cvref_t<decltype((std::make_tuple(TokenKind::Dash, static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1))))>> _match_value; bool _m_matched = false; if (!_m_matched && ((_m >= static_cast<uint8_t>(48) && _m <= static_cast<uint8_t>(57)))) { _match_value.emplace(std::move([&]() { auto end = rusty::iter(rusty::as_bytes(this->stream)).position([&](auto&& b) { return !rusty::is_ascii_digit(b); }).unwrap_or(rusty::len(this->stream));
return std::make_tuple(TokenKind::Digits, std::move(end)); }())); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(45))) { _match_value.emplace(std::move(std::make_tuple(TokenKind::Dash, static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(58))) { _match_value.emplace(std::move(std::make_tuple(TokenKind::Colon, static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(84) || _m == static_cast<uint8_t>(116))) { _match_value.emplace(std::move(std::make_tuple(rusty::clone(rusty::clone(TokenKind::T)), static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(32))) { _match_value.emplace(std::move(std::make_tuple(TokenKind::Space, static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(90) || _m == static_cast<uint8_t>(122))) { _match_value.emplace(std::move(std::make_tuple(rusty::clone(rusty::clone(TokenKind::Z)), static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(43))) { _match_value.emplace(std::move(std::make_tuple(TokenKind::Plus, static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched && (_m == static_cast<uint8_t>(46))) { _match_value.emplace(std::move(std::make_tuple(TokenKind::Dot, static_cast<std::remove_cvref_t<decltype((rusty::len(this->stream)))>>(1)))); _m_matched = true; } if (!_m_matched) { _match_value.emplace(std::move(std::make_tuple(TokenKind::Unknown, rusty::len(this->stream)))); _m_matched = true; } if (!_m_matched) { rusty::intrinsics::unreachable(); } std::move(_match_value).value(); }));
