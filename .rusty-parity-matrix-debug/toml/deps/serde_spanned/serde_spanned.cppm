...
namespace de {
    ...
    struct SpannedDeserializer {
        ...
        template<typename K>
        rusty::Result<rusty::Option<typename K::Value>, E> next_key_seed(K seed) {
            if (this->start.is_some()) {
                ...
                return rusty::Result<rusty::Option<typename K::Value>, E>::Ok(rusty::Option<typename K::Value>(rusty::None));
            }
        }
        template<typename V>
        rusty::Result<typename V::Value, E> next_value_seed(V seed) {
            if (auto&& _iflet_scrutinee = this->start.take(); _iflet_scrutinee.is_some()) {
                ...
                {
                    [&]() -> rusty::Result<typename V::Value, E> { rusty::panicking::panic_fmt(std::string("next_value_seed called before next_key_seed")); }();
                }
            }
        }
    };
