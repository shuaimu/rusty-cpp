    ...
    struct SpannedDeserializer {
        ...
        rusty::Result<rusty::Option<typename K::Value>, Error> next_key_seed(K seed) {
            if (this->start.is_some()) {
                return ::de::rusty_ext::deserialize(seed, std::conditional_t<true, BorrowedStrDeserializer, K>::new_(rusty::clone(rusty::clone(Spanned<T>::START_FIELD)))).map(rusty::Some);
            } else if (this->end.is_some()) {
                return ::de::rusty_ext::deserialize(seed, std::conditional_t<true, BorrowedStrDeserializer, K>::new_(rusty::clone(rusty::clone(Spanned<T>::END_FIELD)))).map(rusty::Some);
            } else if (this->value.is_some()) {
                return ::de::rusty_ext::deserialize(seed, std::conditional_t<true, BorrowedStrDeserializer, K>::new_(rusty::clone(rusty::clone(Spanned<T>::VALUE_FIELD)))).map(rusty::Some);
            } else {
        ...
        rusty::Result<typename V::Value, Error> next_value_seed(V seed) {
            if (auto&& _iflet_scrutinee = this->start.take(); _iflet_scrutinee.is_some()) {
                ...
                return ::de::rusty_ext::deserialize(seed, ::de::value::rusty_ext::into_deserializer(std::move(value)));
            } else {
                {
