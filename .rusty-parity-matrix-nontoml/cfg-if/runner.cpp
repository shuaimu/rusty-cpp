...
namespace tests {
    ...
    using _A = int32_t;
    using _B = int32_t;
    rusty::Option<uint32_t> works1();
    bool works2();
    bool works3();
    rusty::Option<uint32_t> works4();
    bool works5();
    bool works6();
    void it_works();
    void test_usage_within_a_function();
}
...
namespace tests {
    ...
    template<typename... Ts> using Option3 = rusty::Option<Ts...>;




    // Rust-only libtest metadata const skipped: it_works (marker: tests::it_works, should_panic: no)


    // Rust-only libtest metadata const skipped: test_usage_within_a_function (marker: tests::test_usage_within_a_function, should_panic: no)

    // Rust-only trait Trait (Proxy facade emission skipped in module mode)
...
namespace tests {
    ...
    rusty::Option<uint32_t> works1() {
        return rusty::Option<uint32_t>(static_cast<uint32_t>(1));
    }
    ...
    void it_works() {
        if (!works1().is_some()) {
            rusty::panicking::panic("assertion failed: works1().is_some()");
        }
        if (!works2()) {
            rusty::panicking::panic("assertion failed: works2()");
        }
        if (!works3()) {
            rusty::panicking::panic("assertion failed: works3()");
        }
        if (!works4().is_some()) {
            rusty::panicking::panic("assertion failed: works4().is_some()");
        }
        if (!works5()) {
            rusty::panicking::panic("assertion failed: works5()");
        }
        if (!works6()) {
            rusty::panicking::panic("assertion failed: works6()");
        }
    }

    void test_usage_within_a_function() {
        if (!true) {
            rusty::panicking::panic("assertion failed: cfg!(debug_assertions)");
        }
        {
            auto&& _m0_tmp = static_cast<int32_t>(4);
            auto _m0 = &_m0_tmp;
            auto&& _m1_tmp = 2 + 2;
            auto _m1 = &_m1_tmp;
...
namespace tests {
    void Struct::blah() const {
        rusty::panicking::panic("not implemented");
    }
}


// Runnable wrappers for expanded Rust test bodies
// Rust-only libtest wrapper metadata: marker=tests::it_works should_panic=no
void rusty_test_tests_it_works() {
    tests::it_works();
}
// Rust-only libtest wrapper metadata: marker=tests::test_usage_within_a_function should_panic=no
void rusty_test_tests_test_usage_within_a_function() {
    tests::test_usage_within_a_function();
}
...
// Rust-only libtest metadata const skipped: smoke (marker: smoke, should_panic: no)
...
void smoke() {
    if (!::works()) {
        rusty::panicking::panic("assertion failed: works()");
    }
}
...
// Runnable wrappers for expanded Rust test bodies
// Rust-only libtest wrapper metadata: marker=smoke should_panic=no
void rusty_test_smoke() {
    smoke();
}
...
int main(int argc, char** argv) {
    if (argc == 3 && std::string(argv[1]) == "--rusty-single-test") {
        ...
        try {
            if (test_name == "rusty_test_smoke") { rusty_test_smoke(); return 0; }
            if (test_name == "rusty_test_tests_it_works") { rusty_test_tests_it_works(); return 0; }
            if (test_name == "rusty_test_tests_test_usage_within_a_function") { rusty_test_tests_test_usage_within_a_function(); return 0; }
    ...
    int pass = 0, fail = 0;
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_smoke(); std::cout << "  smoke PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  smoke FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  smoke FAILED (unknown exception)" << std::endl; fail++; }
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_tests_it_works(); std::cout << "  tests_it_works PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  tests_it_works FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  tests_it_works FAILED (unknown exception)" << std::endl; fail++; }
    rusty::mem::clear_all_forgotten_addresses();
    try { rusty_test_tests_test_usage_within_a_function(); std::cout << "  tests_test_usage_within_a_function PASSED" << std::endl; pass++; }
    catch (const std::exception& e) { std::cerr << "  tests_test_usage_within_a_function FAILED: " << e.what() << std::endl; fail++; }
    catch (...) { std::cerr << "  tests_test_usage_within_a_function FAILED (unknown exception)" << std::endl; fail++; }
    std::cout << std::endl;
