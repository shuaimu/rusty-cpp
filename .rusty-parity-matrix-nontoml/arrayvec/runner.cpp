    ...
    struct ArrayVecImplRuntimeHelper {
        ...
        static auto pop(auto& self_) -> rusty::Option<typename std::remove_reference_t<decltype(self_)>::Item> {
            ...
            if (rusty::len(self_) == 0) {
                return rusty::Option<typename Self_::Item>(rusty::None);
            }
            // @unsafe
            {
                auto new_len = rusty::len(self_) - 1;
                self_.set_len(std::move(new_len));
                return rusty::Option<typename Self_::Item>(rusty::ptr::read(rusty::ptr::add(rusty::as_ptr(self_), std::move(new_len))));
            }
        }
...
_match_value.emplace(std::move(ch)); } else { if (!(_m.is_none())) { rusty::intrinsics::unreachable(); } return rusty::Option<char32_t>(rusty::None); } std::move(_match_value).value(); });
