                ...
                - [ ] Leaf 4.15.4.3.3.3: Re-run full seven-crate matrix after 4.15.4.3.3.2 and mark Leaf 4 complete only when all seven crates pass
                  ...
                  - [ ] Leaf 4.15.4.3.3.3.3: Re-run full seven-crate matrix after 4.15.4.3.3.3.2 and mark Leaf 4 complete only when all seven crates pass
                    ...
                    - [ ] Leaf 4.15.4.3.3.3.3.3: Re-run full seven-crate matrix after 4.15.4.3.3.3.3.2 and mark Leaf 4 complete only when all seven crates pass
                      ...
                      - [ ] Leaf 4.15.4.3.3.3.3.3.8: Collapse the next deterministic `arrayvec` Stage D iterator-state/runtime-shape frontier after 7.2 (self-field access shadowing in methods, next-iterator adaptation robustness for `for_in`/`map`/`fold`, and first-order dependent runtime shape fallout), then re-run full seven-crate matrix
                        - [ ] Leaf 4.15.4.3.3.3.3.3.8.1: Implement generic transpiler/runtime fixes for the new head blocker family (no crate-specific scripts): preserve `self` field access in assignment-heavy methods (avoid self-shadow emissions like `const auto index = index;`), harden `for_in`/`map`/`fold` next-iterator adaptation for optional-like next results, and collapse the immediate dependent compile-shape failures at this frontier.
                        - [ ] Add fixture-agnostic regressions in `transpiler/src/codegen.rs` and shared runtime headers (leaf4154333333381) covering self-field access emission in mutable methods/DTOR paths and next-iterator adaptation behavior for `fold(map(into_iter(...)))` shapes.
                        - [ ] Verification for 8.1:
                          - `cargo test -p rusty-cpp-transpiler leaf4154333333381 -- --nocapture`
                          - `cargo test -p rusty-cpp-transpiler`
                          - `cargo test`
                        - [ ] Re-run `arrayvec` parity (`tests/transpile_tests/run_parity_matrix.sh --crate arrayvec`) and record whether the 8.1 head blocker family is removed from Stage D first-failure output.
                        - [ ] Leaf 4.15.4.3.3.3.3.3.8.2: Re-run full seven-crate matrix after 8.1 and record the next deterministic blocker (or mark Leaf 4 complete if all pass).
