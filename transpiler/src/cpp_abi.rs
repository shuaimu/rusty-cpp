...
impl<'ast> Visit<'ast> for AttributeCollector<'ast> {
    ...
    fn visit_macro(&mut self, mac: &'ast syn::Macro) {
        if token_stream_mentions_marker(mac.tokens.clone()) {
...
fn reject_collected_markers(
    ...
    if collector.opaque_macro_mentions_marker {
        return Err(format!(
            "reserved cpp_abi/cpp_abi_alias identifier inside opaque macro tokens on {context}"
        ));
    }
    Ok(())
}
...
pub(crate) fn source_mentions_reserved_marker(source: &str) -> bool {
    match syn::parse_file(source) {
        Ok(file) => match collect(&file) {
            Ok(contracts) => !contracts.callables.is_empty() || !contracts.aliases.is_empty(),
...
pub(crate) fn lower(file: &syn::File) -> Result<Option<(syn::File, CppAbiEmissionPlan)>, String> {
    ...
    if contracts.callables.is_empty() {
        return Ok(None);
    }
...
pub(crate) fn preflight_crate_sources(inputs: &[(PathBuf, String)]) -> Result<bool, String> {
    ...
    if units
        .iter()
        .all(|unit| unit.contracts.callables.is_empty() && unit.contracts.aliases.is_empty())
    {
        return Ok(false);
    }
    ...
    for (index, unit) in units.iter().enumerate() {
        ...
        if !unit.contracts.callables.is_empty() || !unit.contracts.aliases.is_empty() {
