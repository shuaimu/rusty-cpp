...
impl CodeGen {
    ...
    pub fn emit_file(&mut self, file: &syn::File, module_name: Option<&str>) {
        ...
        // Pass 1b: collect macro_rules! names so macro-only imports can be skipped.
        self.collect_macro_rules_names(&file.items);
        // Pass 1c: collect globally declared item names for unresolved bare-import filtering.
            ...
            match item {
                syn::Item::Impl(impl_block) => {
                    ...
                    let raw_type_name = tp
                        ...
                        .collect::<Vec<_>>()
                        .join("::");
                    let type_name = qualify_impl_type_name(&raw_type_name, module_path);
    ...
    fn emit_struct(&mut self, s: &syn::ItemStruct) {
        ...
        if !named_field_types.is_empty() {
            ...
            self.struct_field_types.insert(scoped_name, named_field_types);
        }
            ...
            match &arm.pat {
                ...
                syn::Pat::Or(or_pat) => {
                    ...
                    for case in &or_pat.cases {
                        match case {
                            syn::Pat::Lit(lit) => conds.push(format!("_m == {}", self.emit_lit(&lit.lit))),
                            syn::Pat::Path(pp) => {
        ...
        for (idx, arm) in match_expr.arms.iter().enumerate() {
            ...
            match &arm.pat {
                syn::Pat::TupleStruct(ts) => {
                    ...
                    out.push_str(&format!("auto {} = _m.{}(); ", matched_value, unwrap_method));
    ...
    fn expected_type_template_args_from_path(
        &self,
        tp: &syn::TypePath,
    ) -> Option<Vec<String>> {
        let last = tp.path.segments.last()?;
    ...
    fn emit_expr_to_string(&self, expr: &syn::Expr) -> String {
        match expr {
            ...
            syn::Expr::Return(ret) => {
                ...
                match &ret.expr {
                    Some(e) => {
                        let val = self.emit_expr_to_string_with_expected(e, self.current_return_type_hint());
                        format!("{} {}", keyword, val)
                    }
...
fn qualify_impl_type_name(raw: &str, module_path: &[String]) -> String {
