...
export module vec_deque_port.drain;

namespace vec_deque_port::drain {
...
struct Drain {
    ...
    static Drain<T, A> new_(rusty::VecDeque<T, A>& deque, size_t drain_start, size_t drain_len) {
        const auto orig_len = rusty::mem::replace(deque.len, std::move(drain_start));
    ...
    struct DropGuard {
        ...
        ~DropGuard() noexcept(false) {
            ...
            const auto drain_len = this->_0.drain_len;
    ...
    rusty::Option<T> next() {
        ...
        const auto wrapped_idx = this->deque.as_ref().to_physical_idx(this->idx);
        this->idx += 1;
        this->remaining -= 1;
    ...
    rusty::Option<T> next_back() {
        ...
        this->remaining -= 1;
        const auto wrapped_idx = this->deque.as_ref().to_physical_idx(rusty::detail::deref_if_pointer_like(this->idx) + rusty::detail::deref_if_pointer_like(this->remaining));
        return rusty::Option<T>(this->deque.as_mut().buffer_read(std::move(wrapped_idx)));
    }
    template<typename I>
    bool fill(I& replace_with) {
        ...
        for (auto&& idx : rusty::for_in(rusty::range(range_start, range_end))) {
            if (auto&& _iflet_scrutinee = ([&](auto&& __recv) -> decltype(auto) { if constexpr (requires { std::forward<decltype(__recv)>(__recv).next(); }) { return std::forward<decltype(__recv)>(__recv).next(); } else { return std::forward<decltype(__recv)>(__recv)->next(); } }(replace_with)); _iflet_scrutinee.is_some()) {
                ...
                const auto index = deque.to_physical_idx(std::move(idx));
                // @unsafe
                {
    ...
    void move_tail(size_t additional) {
        rusty::VecDeque<T, A>& deque = this->deque.as_mut();
        const auto tail_start = rusty::detail::deref_if_pointer_like(deque.len) + rusty::detail::deref_if_pointer_like(this->drain_len);
        ...
        const auto new_tail_start = rusty::detail::deref_if_pointer_like(tail_start) + rusty::detail::deref_if_pointer_like(additional);
        // @unsafe
        {
