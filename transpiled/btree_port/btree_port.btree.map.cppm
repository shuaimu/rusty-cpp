...
namespace rusty::port::collections::btree::map::entry {}
...
struct ExtractIfInner {
    ...
    ::rusty::Option<std::tuple<const K&, const V&>> peek() const {
        const auto edge = RUSTY_TRY_OPT(this->cur_leaf_edge.as_ref());
    ...
    ::rusty::Option<std::tuple<K, V>> next(F& pred, A alloc) {
        while (true) {
            ...
            {
                auto&& _m = ([&](auto&& __recv) -> decltype(auto) { if constexpr (requires { std::forward<decltype(__recv)>(__recv).end_bound(); }) { return std::forward<decltype(__recv)>(__recv).end_bound(); } else { return std::forward<decltype(__recv)>(__recv)->end_bound(); } }(this->range));
...
return ::rusty::Option<std::tuple<K&, V&>>{::rusty::None}; }(); } return [&]() -> ::rusty::Option<std::tuple<K&, V&>> { ::rusty::intrinsics::unreachable(); }(); }();
    ...
    ::rusty::Option<std::tuple<K&, V&>> peek_next() {
        const auto current = RUSTY_TRY_OPT(this->current.as_mut());
...
return root.push_internal_level(::rusty::clone(this->alloc)).push(std::move(([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._0; }) return (std::forward<decltype(__t)>(__t)._0); else return std::get<0>(std::forward<decltype(__t)>(__t)); })(ins.kv)), std::move(([&](auto&& __t) -> decltype(auto) { if constexpr (requires { __t._1; }) return (std::forward<decltype(__t)>(__t)._1); else return std::get<1>(std::forward<decltype(__t)>(__t)); })(ins.kv)), std::move(ins.right));
    ...
    auto insert_after(K key, V value) -> ::rusty::Result<std::tuple<>, UnorderedKeyError> {
        if (auto&& _iflet_scrutinee = this->peek_prev(); _iflet_scrutinee.is_some()) {
            ...
            if (key <= ::rusty::detail::deref_if_pointer_like(prev)) {
                return ::rusty::Result<std::tuple<>, UnorderedKeyError>::Err(UnorderedKeyError{});
            }
        }
        ...
        return ::rusty::Result<std::tuple<>, UnorderedKeyError>::Ok(std::make_tuple());
    }
    auto insert_before(K key, V value) -> ::rusty::Result<std::tuple<>, UnorderedKeyError> {
        if (auto&& _iflet_scrutinee = this->peek_prev(); _iflet_scrutinee.is_some()) {
            auto&& _iflet_payload = _iflet_scrutinee.unwrap();
            auto&& prev = ::rusty::detail::deref_if_pointer(std::get<0>(::rusty::detail::deref_if_pointer(_iflet_payload)));
            if (key <= ::rusty::detail::deref_if_pointer_like(prev)) {
...
return std::move(k);
    ...
    template<typename T>
    auto insert_after(T value) -> ::rusty::Result<std::tuple<>, UnorderedKeyError> {
        return this->inner.insert_after(std::move(value), SetValZST);
    }
    template<typename T>
    auto insert_before(T value) -> ::rusty::Result<std::tuple<>, UnorderedKeyError> {
...
} }(); } if (_m.is_some()) { auto&& _mv1 = _m.unwrap(); auto&& handle = ::rusty::detail::deref_if_pointer(_mv1); return handle.insert_recursing(std::move(this->key_field), std::move(value), ::rusty::clone(this->alloc), [&](auto&& ins) {
::rusty::mem::drop(std::move(ins.left));
...
{
    auto& root = out_tree.root.as_mut().unwrap();
...
{
    auto& out_root = out_tree.root.as_mut().unwrap();
...
return VacantEntry{.key = std::move(key), .handle = ::rusty::Some(handle), .dormant_map = std::move(dormant_map), .alloc = ::rusty::clone(((::rusty::detail::deref_if_pointer_like(map.alloc)))), ._marker = ::rusty::PhantomData<std::tuple<>>{}}.insert_entry(btree_internal::SetValZST{}).into_key(); }(); } return [&]() -> const K& { ::rusty::intrinsics::unreachable(); }(); }();
    }
#endif
    ...
    ::rusty::Option<entry::OccupiedEntry<K, V, A>> first_entry() {
        ...
        return ::rusty::Option<entry::OccupiedEntry<K, V, A>>(entry::OccupiedEntry<K, V, A>{.handle = kv.forget_node_type(), .dormant_map = std::move(dormant_map), .alloc = ::rusty::clone(((::rusty::detail::deref_if_pointer_like(map.alloc)))), ._marker = ::rusty::PhantomData<std::tuple<>>{}});
    }
...
return std::move(v);
    ...
    BTreeMap<K, V, A> split_off(const Q& key) {
        ...
        auto& left_root = this->root.as_mut().unwrap();
    ...
    std::tuple<ExtractIfInner<K, V, R>, A> extract_if_inner(R range) {
        if (auto&& _iflet_scrutinee = this->root.as_mut(); _iflet_scrutinee.is_some()) {
            ...
            auto first = root_shadow1.borrow_mut().lower_bound(btree_internal::SearchBound<K>::from_range(([&](auto&& __recv) -> decltype(auto) { if constexpr (requires { std::forward<decltype(__recv)>(__recv).start_bound(); }) { return std::forward<decltype(__recv)>(__recv).start_bound(); } else { return std::forward<decltype(__recv)>(__recv)->start_bound(); } }(range))));
...
_match_value.emplace(std::move(root.reborrow())); } else { if (!(_m.is_none())) { ::rusty::intrinsics::unreachable(); } return Cursor<K, V>{.current = ::rusty::Option<btree_internal::Handle<btree_internal::NodeRef<marker::Immut, K, V, marker::Leaf>, marker::Edge>>{::rusty::None}, .root = ::rusty::Option<const node::Root<K, V>&>{::rusty::None}}; } ([&](auto&& __v) -> decltype(auto) { using __MatchValueT = std::remove_cvref_t<decltype(__v)>; if constexpr (requires { typename __MatchValueT::type; }) { if constexpr (std::is_same_v<__MatchValueT, std::reference_wrapper<typename __MatchValueT::type>>) { return std::forward<decltype(__v)>(__v).get(); } else { return std::forward<decltype(__v)>(__v); } } else { return std::forward<decltype(__v)>(__v); } })(std::move(_match_value).value()); });
        auto edge = root_node.lower_bound(btree_internal::SearchBound<K>::from_range(bound));
