...
using Rc = rusty::Arc<T>;
constexpr uint16_t SHARED_ITER_MAX = static_cast<uint16_t>(100);
namespace pin_coerce_unsized {
// [arc_tests_port] dropped (pin-coercion helper)
// [arc_tests_port] dropped (pin-coercion helper)
}
template<typename I>
...
using rusty::Arc;
// Rust-only: using std::sync::UniqueArc;
using rusty::sync::Weak;
...
TEST_CASE("uninhabited") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP uninhabited: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("slice") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP slice: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("trait_object") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP trait_object: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("float_nan_ne") {
    const auto x = rusty::Arc<float>::new_(std::numeric_limits<float>::quiet_NaN());
...
TEST_CASE("shared_from_iter_normal") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP shared_from_iter_normal: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("shared_from_iter_trustedlen_normal") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP shared_from_iter_trustedlen_normal: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("shared_from_iter_trustedlen_panic") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP shared_from_iter_trustedlen_panic: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("shared_from_iter_trustedlen_no_fuse") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP shared_from_iter_trustedlen_no_fuse: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("weak_may_dangle") {
    const rusty::SafeFn<rusty::sync::Weak<std::string_view>(rusty::sync::Weak<std::string_view>&)> hmm = +[](rusty::sync::Weak<std::string_view>& val) -> rusty::sync::Weak<std::string_view> {
...
TEST_CASE("make_mut_unsized") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP make_mut_unsized: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}

TEST_CASE("test_unique_arc_weak") {
    /* PATCHED: skipped */
    std::printf("[port] SKIP test_unique_arc_weak: Arc<[T]>/Arc<dyn>/UniqueArc (DST) not in port\n");
    return;
}
