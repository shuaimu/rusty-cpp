...
// Rust-only: using std::iter::SourceIter;
// Rust-only: using std::iter::TrustedFused;
...
struct Hole {
    ...
    void move_to(size_t index) {
        ...
        // @unsafe
        {
            ...
            const auto hole_ptr = rusty::ptr::add(ptr_shadow1, this->pos_field);
...
struct IntoIter {
    ...
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return f.debug_tuple("IntoIter").field(rusty::as_slice(this->iter)).finish();
    }
    rusty::Option<T> next() {
        return this->iter.next();
    }
    ...
    IntoIter clone() const { return IntoIter{.iter = rusty::clone(this->iter)}; }
};
...
struct Drain {
    ...
    friend std::ostream& operator<<(std::ostream& os, const Drain& v) {
        return os << "Drain { ... }";
    }
};
...
struct BinaryHeap {
    ...
    rusty::Option<PeekMut<T, A>> peek_mut() {
        if (rusty::is_empty((*this))) {
            ...
            return rusty::Option<PeekMut<T, A>>(PeekMut<T, A>((*this), rusty::Option<rusty::num::NonZero<size_t>>{rusty::None}));
        }
    }
    rusty::Option<T> pop() {
...
return std::move(item);
    ...
    void rebuild_tail(size_t start) {
        ...
        const auto tail_len = rusty::len((*this)) - rusty::detail::deref_if_pointer_like(start);
        const auto better_to_rebuild = (rusty::detail::deref_if_pointer_like(start) < rusty::detail::deref_if_pointer_like(tail_len) ? true : (rusty::len((*this)) <= 2048 ? (2 * rusty::len((*this))) < (rusty::detail::deref_if_pointer_like(tail_len) * log2_fast(std::move(start))) : (2 * rusty::len((*this))) < (rusty::detail::deref_if_pointer_like(tail_len) * 11)));
        if (better_to_rebuild) {
            this->rebuild();
        } else {
            for (auto&& i : rusty::for_in(rusty::range(start, rusty::len((*this))))) {
                // @unsafe
                {
    ...
    void append(BinaryHeap<T, A>& other) {
        ...
        auto start = rusty::len(this->data);
    ...
    template<typename F>
    void retain(F f) {
        auto guard = RebuildOnDrop<T, A>((*this), rusty::len((*this)));
        auto i = 0;
        guard.heap.data.retain([&](auto&& e) {
auto keep = f(std::move(e));
if (!keep && (rusty::detail::deref_if_pointer_like(i) < rusty::detail::deref_if_pointer_like(guard.rebuild_from))) {
...
return std::move(keep);
    ...
    rusty::Option<const T&> peek() const {
        return rusty::get(this->data, static_cast<size_t>(0));
    }
    ...
    template<size_t N>
    static BinaryHeap<T, A> from(std::array<T, rusty::sanitize_array_capacity<N>()> arr) {
    ...
    template<typename I>
    void extend(I iter) {
        const auto guard = RebuildOnDrop<T, A>((*this), rusty::len((*this)));
...
struct PeekMut {
    ...
    rusty::fmt::Result fmt(rusty::fmt::Formatter& f) const {
        return f.debug_tuple("PeekMut").field(&this->heap.data[0]).finish();
    }
    ...
    const T& operator*() const {
        assert((!rusty::is_empty(this->heap)));
        // @unsafe
        {
    ...
    T& operator*() {
        assert((!rusty::is_empty(this->heap)));
        const auto len = rusty::len(this->heap);
...
struct IntoIterSorted {
    ...
    std::tuple<size_t, rusty::Option<size_t>> size_hint() const {
        auto exact = rusty::len(this->inner);
        return std::make_tuple(std::move(exact), rusty::Option<size_t>(std::move(exact)));
    }

    IntoIterSorted clone() const { return IntoIterSorted{.inner = rusty::clone(this->inner)}; }
    friend std::ostream& operator<<(std::ostream& os, const IntoIterSorted& v) {
        return os << "IntoIterSorted { ... }";
    }
};
...
struct DrainSorted {
    ...
    friend std::ostream& operator<<(std::ostream& os, const DrainSorted& v) {
        return os << "DrainSorted { ... }";
    }
};
